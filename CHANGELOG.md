...
## v1.11.0 — Comics Tracker et LibGen

### Deux nouvelles sources

**Comics Tracker** : des comics américains en **éditions françaises** — DC, Marvel, collections type DC Black Label. Une recherche rend une soixantaine d'éditions françaises en moins d'une seconde, avec leur collection et leurs auteurs. Le rayon Comics en profite seul : le site n'a pas de bande dessinée franco-belge.

Elle demande **votre compte du site**, à saisir dans Compte : adresse et mot de passe y sont chiffrés comme vos clés de débrideur, et la connexion se fait depuis le serveur. Les fichiers viennent ensuite de Google Drive, qui les sert par morceaux à 1,2 Mo/s : un album de 200 Mo s'ouvre en quatre secondes, une planche arrive en une seconde. Rien n'est rapatrié.

Le site mélange les formes : la plupart des éditions sont des CBZ, qui se lisent en ligne ; certaines sont des RAR compressés, qui se téléchargent. La liste n'annonce donc aucun format — il est lu à l'ouverture, pas deviné.

**LibGen** : des **livres et de la presse en français**, lisibles sans aucun débrideur. Michel Bussi rend 41 résultats français, *Le Monde diplomatique* une centaine de numéros, datés et rangés du plus récent au plus ancien.

Son serveur de fichiers est lent — entre 14 et 400 Ko/s selon le moment — et lire une planche à distance demandait vingt secondes. Le fichier est donc **préchargé sur le serveur**, avec une barre d'avancement : le téléchargement continue si vous fermez la page, reprend à l'octet près après une coupure, et la lecture se fait ensuite sur la copie locale, instantanée. Dix gigaoctets sont gardés, les fichiers les moins récemment lus laissant la place.

Ses comics et ses mangas ne sont pas proposés : leurs fichiers ne s'ouvrent plus. Sur des fichiers pris au hasard et réellement sollicités, la presse et les livres sont servis neuf fois sur dix, les comics deux fois sur cinq, les mangas jamais.

### Améliorations

- **Les numéros de presse sont classés du plus récent au plus ancien**, quelle que soit la source. Une date se trie, contrairement à un titre : « 2 septembre » vient bien après « 26 août ».
- **Une œuvre, une carte dans le catalogue.** Chercher « Le Parisien » rendait plusieurs cartes identiques, une par notice de la BnF, qui menaient toutes au même endroit.
- **Une archive RAR compressée est refusée avant le clic**, avec sa raison, en quelques secondes au lieu de deux minutes et demie : le verdict était déjà acquis à la première planche.
- **Télécharger un fichier d'une source qui le sert elle-même** ne réclame plus de clé de débrideur — fourtoutici, LibGen et Comics Tracker n'en ont besoin d'aucune.
- **Un transfert lent mais qui avance n'est plus coupé** : la lecture n'abandonne que sur un vrai silence du serveur.

## v1.10.0 — worldivx, la presse en torrents

### Une nouvelle source

**worldivx** : 2 327 numéros de presse en PDF — *Le Monde*, *Libération*, *Les Échos*, *L'Équipe* au jour le jour — et 278 livres, surtout en epub. Elle alimente Presse et Livres, le numéro le plus récent en tête.

C'est un site public francophone, et ce sont des **torrents à trackers publics** : chaque résultat arrive avec ses pastilles de cache AllDebrid et TorBox, se lit comme n'importe quel torrent en cache, et aucun ratio n'est en jeu. C'est ce qui la distingue de downmagaz et de 1001ebooks, qui servent la même presse en liens directs.

Son inventaire est construit à partir de ses pages de liste — 53 pages et 87 secondes la première fois, deux pages par rafraîchissement — plutôt qu'avec sa recherche, qui mélange toutes les catégories et plafonne à cinquante lignes.

## v1.9.0 — 1001ebooks et downmagaz, la presse au jour le jour

### Deux nouvelles sources

**downmagaz** : 123 929 fiches, presque toute la presse française en PDF — *Le Monde*, *Investir*, *Air & Cosmos* au jour le jour — et 8 566 albums de BD. Elle alimente Presse, BD et Comics, et **se lit en ligne avec AllDebrid**. Le dernier numéro paru arrive en tête.

**1001ebooks** : 47 501 fiches, 43 528 livres et près de 4 000 numéros de presse répartis sur 888 périodiques. Le fichier est derrière le guichet du site : la carte propose « Voir sur 1001ebooks », et le téléchargement se fait chez lui, avec votre compte.

Les deux sites ferment leur recherche aux robots. Leur inventaire est donc construit à partir de leur sitemap, que leur `robots.txt` autorise, et la recherche se fait dans cet index local. Le sitemap de downmagaz est figé depuis novembre 2025 : ses pages de liste comblent les dix mois manquants.

### Corrections

- **Les liens directs ne sont plus proposés qu'au débrideur qui sait les ouvrir.** L'application ne débloque un lien d'hébergeur qu'avec AllDebrid, mais comptait TorBox comme capable — d'où « vos 2 débrideurs n'a su ouvrir aucun lien » sur un lien vivant. Et la liste d'hébergeurs d'AllDebrid, incomplète, servait de barrière : elle ordonne désormais les essais sans en interdire aucun, et c'est sa réponse qui tranche. Wawacity et bookys en profitent aussi.
- **AllDebrid ignorait les domaines secondaires qu'il déclare** (`htfl.net`, `turbobit.cloud`…).
- **La liseuse PDF zoomait sans permettre de défiler**, surtout sur téléphone, où le zoom ne changeait rien du tout.
- « Air amp cosmos » redevient « Air & Cosmos ».

## v1.8.0 — fourtoutici, et un kiosque qui dit vrai

### Une source qui ne demande aucune clé — et se lit sans débrideur

**fourtoutici** entre au catalogue. Elle est la première source du projet à ne rien demander : ni compte, ni clé, ni débrideur. Elle sert ses fichiers elle-même et honore les plages HTTP, donc un livre ou un magazine s'y **lit en ligne** directement — jusqu'ici seul Telegram le permettait.

Son API annonce l'extension, la taille et le type de chaque fichier : rien n'est deviné depuis le nom, contrairement à toutes les autres sources en lien direct. Elle alimente Livres, Mangas, BD, Comics, Presse et Audio.

**Deux limites, mesurées.** Le site refuse tout ce qui n'est pas un mot simple sans accent : la recherche n'envoie donc que le mot le plus significatif du titre. Et il classe par pertinence en pages de cinquante — chercher le magazine « Elle » rendait 297 fichiers sans jamais montrer les deux seuls magazines. La recherche filtre maintenant **par catégorie chez lui**, et suit ses pages.

### Le kiosque presse

« Vient de sortir » vient enfin des sources, après avoir été retiré parce qu'il ne venait de nulle part. Quatre onglets plutôt que six, parce que les sources n'en distinguent pas six. Le filtre de format existait, il est désormais appliqué. Et une release n'est plus prise pour une œuvre.

### Administration

L'adresse d'une source se corrige depuis l'application, sans reconstruire l'image.

### Corrections

- **Un EPUB n'est pas un CBZ.** C'est un ZIP : rien dans ses premiers octets ne l'en distingue. Il partait donc à la visionneuse, qui y trouvait la couverture et une illustration et annonçait « 2 planches » pour quatre cents pages. Il est maintenant reconnu à ses entrées et ouvert dans la liseuse.
- **« Erreur 502 » à l'ouverture d'un EPUB** — et le 502 mentait : toute panne était aplatie sur ce code, une erreur de contrat comprise.
- **« Source fourtoutici inconnue »** sur une release parfaitement lisible.
- **Un numéro de presse n'est pas un tome.** « Mad Movies 406 » entrait comme le tome 406 : l'étagère dessinait 406 cases, dont 405 en pointillé pour des numéros que personne n'a jamais eus, et « Reprendre » en ajoutait une 407ᵉ. Un périodique porte désormais le nom de son fichier, la seule identité datable — et les positions déjà écrites sont reprises au démarrage.
- **La pastille du numéro lu restait en pointillé.** Trois surfaces de lecture sur quatre recevaient « ce que sert cette release » et le jetaient en silence.
- **La liste des numéros disponibles** passe au second rang d'une carte de presse, là où un périodique la réclame.

## v1.7.0 — La presse, zone-ebook, et cinq failles refermées

### Un rayon Presse, avec les mêmes règles que les autres

Journaux et magazines ont désormais leur rayon, servi par la BnF au catalogue et par quatre trackers, wawacity, bookys et zone-ebook. Le kiosque monte dans la barre du haut : un périodique se **regarde** avant de se chercher.

**Un numéro n'est pas un tome, et c'était toute la difficulté.** Le parseur de release lisait le quantième du mois comme un volume — « Le Journal de Spirou N°4611 du 26 août 2026 » devenait le tome 26 — et l'étagère se remplissait de tomes qui n'existaient pas. Un numéro porte maintenant sa **date**, lue par le parseur du kiosque et par lui seul.

**Et un périodique n'a pas une release, il en a une par parution.** Choisir un numéro le rattache à lui-même plutôt que de remplacer le raccourci de l'œuvre : les numéros s'accumulent dans l'étagère au lieu de se chasser l'un l'autre, du plus récent au plus ancien.

### zone-ebook cherche, mais seulement pour ses membres

Sa recherche répondait toujours la même chose à toutes les questions. Mesuré anonymement, le verdict était juste ; mesuré connecté, il s'inverse — trois requêtes rendent trois jeux de résultats différents. Ce n'était pas « ce site ne sait pas chercher » mais « ce site ne cherche pas pour les visiteurs ».

Les identifiants se saisissent dans « Compte », chiffrés par la clé dérivée de votre mot de passe, et **c'est le pseudo qu'il faut, jamais l'adresse** : le site refuse la connexion par adresse. La recherche interroge les six rayons, en demandant la catégorie au site plutôt qu'en triant après, et lit jusqu'à quatre pages.

### Cinq failles refermées, aucune critique

Un pentest complet a été mené avant cette publication. Toutes les failles trouvées étaient de gravité moyenne, et **quatre sur cinq consistaient à ne pas avoir appliqué une règle déjà écrite ailleurs dans le projet**.

- **CSRF sur les surfaces de lecture.** Ce sont des `GET`, et elles débrident : un lien piégé suffisait à dépenser le quota AllDebrid ou Z-Library d'un utilisateur connecté. Elles ne peuvent pas devenir des `POST` — pdf.js demande ses plages en `GET` — donc la garde est la provenance déclarée par le navigateur.
- **SSRF par l'adresse Rustatio**, seul champ du projet à atteindre la position d'hôte d'un appel sortant, et qui n'était vérifié que sur sa forme.
- **SSRF par l'URL du `.torrent`** : cinq endroits la récupèrent, trois ne la vérifiaient pas.
- **Assainisseur EPUB contournable** de trois façons indépendantes, chacune reproduite en exécutant la fonction. La politique de sécurité du contenu les bloquait toutes, mais l'assainisseur est la garantie que le code documente.

### Lecture

Les PDF s'ouvrent depuis la bibliothèque, plus seulement depuis la recherche. La liseuse EPUB dit son attente et la compte en secondes, au lieu d'une page blanche. Et quand aucun débrideur ne sait ouvrir un lien, un bouton mène à l'hébergeur plutôt qu'à une impasse — le refus porte désormais le nom de celui qui l'a prononcé, jamais « lien mort ».
