...
/**
 * LE DOMAINE VIENT DES REGLAGES. Il etait ecrit en dur, « www7 » compris — et c'est
 * precisement ce prefixe qui tourne chez ce site.
 */
function BASE_(): string {
  return getSettings().ddl.bookys.base.replace(/\/+$/, '');
}
  ...
  return {
    ...
    async search(q: Query, ctx: SearchContext): Promise<Candidate[]> {
      ...
      const html = await page(
        `${BASE_()}/search?q=${encodeURIComponent(terme)}`,
     ...
     *
     * LA PAGE DE SECTION, PAS LA RECHERCHE. Elle passe par le meme `page()` — donc par
     * flaresolverr, puisque bookys repond 403 a une requete nue (verifie le 2026-09-02).
     * C'est ce qui rend cette source lente, et pourquoi elle garde son budget propre.
     *
     * LE MEME PARSEUR QUE LA RECHERCHE : une page de section liste ses fiches sous la
     * meme forme qu'un resultat. Un second parseur aurait diverge du premier.
     */
    async dernieres(ctx): Promise<Candidate[]> {
      ...
      for (const section of sections) {
        ...
        const html = await page(`${BASE_()}/${section}`, `bookys:section:${section}`, ctx.signal);
        // Echec, pas section vide : voir la note de `search`.
        if (html === null) throw new Error("bookys n'a rendu aucune reponse exploitable");
        for (const c of parseRechercheBookys(html, [section])) {
...
export async function ouvrirFicheBookys(
  ...
  const html = await page(`${BASE_()}/bandes-dessinees/${id}-x`, `bookys:fiche:${id}`, signal);
