...
import { postForm, getText } from '../../core/http';
...
import { baseZoneEbook } from '../../presse/zone-ebook';
import type { Candidate, Query, SearchContext, Source } from '../types';

// LE DOMAINE VIT DANS `presse/zone-ebook.ts`, et il n'est declare qu'une fois : il
// l'etait ici AUSSI, en dur, et les deux auraient diverge au premier changement.
...
async function pageDeRecherche(
  ...
  return postForm(
    `${baseZoneEbook()}/index.php?do=search`,
    {
      do: 'search',
      subaction: 'search',
    ...
    },
    {
      ...
      signal,
      maxBytes: MAX_OCTETS,
      headers: { Cookie: cookie, Referer: `${baseZoneEbook()}/index.php?do=search` },
    },
  );
}
...
export function zoneEbookSource(): Source {
  return {
     ...
     *
     * LA PAGE DE RUBRIQUE, PAS LA RECHERCHE. Mesure du 2026-09-02 : `/bd-comics-mangas/`
     * rend onze fiches publiquement, sans session — alors que la RECHERCHE du meme site
     * n'existe que pour ses membres. Les deux portes n'ont pas les memes exigences, et
     * parcourir se paie donc sans identifiants.
     *
     * LE MEME PARSEUR QUE LA RECHERCHE : les liens d'une page de rubrique ont la forme des
     * liens d'un resultat. En ecrire un second l'aurait fait diverger du premier — la
     * faute payee quatre fois cette semaine.
     */
    async dernieres(ctx): Promise<Candidate[]> {
      const sections = SECTIONS_PAR_FAMILLE[ctx.famille.id];
      if (!sections) return [];
      ...
      for (const section of sections) {
        ...
        const html = await getText(urlSection(section), {
          timeoutMs: 15000, signal: ctx.signal, maxBytes: MAX_OCTETS,
        });
        // ECHEC, PAS RUBRIQUE VIDE : la distinction remonte jusqu'a la pastille du site.
        if (html === null) throw new Error("zone-ebook n'a rendu aucune reponse exploitable");
        for (const c of parseRechercheZone(html, [section])) {
          if (c.identite && !parId.has(c.identite)) parId.set(c.identite, c);
        }
      }
      return [...parId.values()].slice(0, MAX_RESULTATS);
 ...
 *
 * MESURE DU 2026-09-02 : `/bd-comics-mangas/` rend onze fiches datees, publiquement.
 */
export function urlSection(section: string, page = 1): string {
  return page <= 1 ? `${baseZoneEbook()}/${section}/` : `${baseZoneEbook()}/${section}/page/${page}/`;
}
