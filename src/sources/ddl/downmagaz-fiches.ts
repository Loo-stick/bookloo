...
/**
 * Lit les pages de liste, de la plus recente vers les plus anciennes, jusqu'a retomber
 * sur du connu.
 *
 * S'ARRETER APRES PLUSIEURS PAGES CONNUES, PAS UNE : l'ordre des pages n'est pas strict,
 * une fiche modifiee remontant en tete. Mesure du 2026-09-11 : la page 2 descendait
 * jusqu'au numero 124 257 quand la page 3 commencait a 124 366.
 *
...
export async function parcourirListes(o: ParcoursListes): Promise<{ pages: number; nouvelles: number }> {
  ...
  for (let n = 1; n <= o.maxPages && dejaConnues < o.arretApres; n += 1) {
    ...
    if (html === null) break;
    const fiches = fichesDeLaListe(html);
    if (!fiches.length) break;
    pages += 1;
    const nouvelles = o.connait ? fiches.filter((f) => !o.connait!(f.rubrique, f.slug)) : fiches;
