...
// Les cles qu'une SOURCE peut exiger. Ce sont des passkeys de trackers, plus les
// identifiants de zone-ebook, seul site dont la RECHERCHE demande un compte.
export type NomCle = 'c411' | 'tr4ker' | 'ygg' | 'g3mini' | 'v3x' | 'zone_login' | 'zone_mdp'
  // Comics Tracker : le mot de passe suffit a dire qu'un compte est configure.
  | 'ct_email' | 'ct_mdp';
...
export interface Candidate {
  ...
  edition?: string;
  /**
   * La parution d'un numero de presse : son libelle (« 8 septembre 2026 ») et sa date
   * TRIABLE (`2026-09-08`, ou `2026-09` pour un mensuel).
   *
   * Posees par le flux pour une famille qui ne compte pas en tomes, a la place des tomes
   * que le parseur de release inventerait. La date seule se trie — « 2 septembre » passe
   * apres « 26 aout » dans l'ordre alphabetique — et c'est elle qui range les numeros du
   * plus recent au plus ancien. Absentes : ce n'est pas un numero, ou sa date ne se lit pas.
   */
  parution?: string;
  paruLe?: string;
...
export interface SearchContext {
   ...
   * QUI cherche.
   *
   * Une source qui ouvre une session la retient par utilisateur — deux personnes n'ont ni
   * le meme compte ni le meme quota. Les sources qui n'en ouvrent pas l'ignorent.
   */
  userId?: number;
  /**
   * Famille interrogee.
...
/**
 * Ce qu'une source rend quand on ne cherche RIEN.
 *
 * PARCOURIR N'EST PAS CHERCHER, et c'est pourquoi c'est une methode a part plutot qu'une
 * requete vide. `search` recoit une `Query` et la plupart des sources commencent par
 * `if (!terme) return []` — leur passer du vide rendrait donc zero, en silence, et se
 * lirait « rien n'est sorti ».
 *
 * FACULTATIVE : une source qui ne sait pas parcourir ne la declare pas, et le kiosque le
 * DIT au lieu de montrer une section vide.
 */
...
export interface Source {
  ...
  cleRequise?: NomCle;
  /**
   * Les derniers ajouts de cette source pour ce rayon, sans requete.
   *
   * Le contrat est celui de `search` : une liste vide dit « j'ai repondu, rien de neuf ».
   * Un echec LEVE. Les confondre afficherait « rien n'est sorti » pendant une panne.
   */
  dernieres?(ctx: ContexteParcours): Promise<Candidate[]>;
  /**
   * Le budget de CETTE source, quand il n'a rien a voir avec celui des autres.
   *
   * Absent, le budget commun du fan-out s'applique — c'est le cas de toutes les sources
   * qui interrogent une API. bookys passe par un defi Cloudflare et met quatorze secondes
   * la ou un tracker en met deux : sous le budget commun il etait abandonne a chaque
   * recherche. Les resultats partant au fil de l'eau, un budget plus long ne fait
   * attendre personne — il laisse seulement la source lente completer la liste.
   */
  budgetMs?: number;
