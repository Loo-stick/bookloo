...
function fabriquerSource(id: string, label: string, cleRequise: NomCle): Source {
  return {
    ...
    /**
     * Les derniers ajouts du tracker pour ce rayon.
     *
     * `t=search` SANS `q`, ET AVEC LES CATEGORIES DE LA FAMILLE. Torznab rend alors les
     * entrees les plus recentes de ces categories — c'est la meme requete que la
     * recherche, moins le terme.
     *
     * SANS CATEGORIE, ON NE DEMANDE RIEN. `construireUrl` retombe sur celles de
     * l'indexeur quand la famille n'en declare aucune : pour une recherche c'est un repli
     * utile, ici cela rapatrierait le tracker ENTIER, toutes familles melangees. Une
     * famille non declaree n'est donc pas parcourue du tout.
     */
    async dernieres(ctx): Promise<Candidate[]> {
      ...
      const apiKey = ctx.cles[cleRequise];
      if (!apiKey) return [];
      if (sourceEcartee(ctx.famille, id)) return [];

      const categories = categoriesPour(ctx.famille, id);
      if (categories.length === 0) return [];

      const items = await recuperer(
        id, indexeur, apiKey, { t: 'search' }, categories, ctx.signal,
      );
      // ECHEC, PAS RESULTAT VIDE : meme regle qu'au-dessus, et pour la meme raison.
      if (items === null) throw new Error(`${label} n'a rendu aucune reponse exploitable`);
