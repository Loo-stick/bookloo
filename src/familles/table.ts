...
export type IdFamille = 'mangas' | 'comics' | 'bd' | 'livres' | 'audio' | 'presse';
...
export interface Famille {
   ...
   */
  catalogues: ('mangadex' | 'comicvine' | 'openlibrary' | 'booknode' | 'bnf')[];
...
export const FAMILLES: Famille[] = [
  ...
  },
  {
    id: 'presse',
    libelle: 'Presse',
    // LA BnF EST LE SEUL CATALOGUE DE PERIODIQUES. Les quatre autres decrivent des oeuvres
    // a tomes ; un numero de journal est un titre et une date. Son catalogue general nomme
    // le PERIODIQUE — titre canonique, ISSN, plage de parution — ce qui donne enfin a
    // « Le Monde » une identite stable, la ou il n'y avait que la graphie du site qui
    // l'avait publie.
    //
    // Elle ne rend PAS les numeros : ceux-la continuent de venir des sources. Le catalogue
    // nomme, les sources fournissent — c'est deja la repartition partout ailleurs.
    catalogues: ['bnf'],
    // 7010 « Books/Mags » EST LA CATEGORIE STANDARD Torznab pour la presse. Le commentaire
    // de la famille Mangas le dit deja en creux : « 7000 est la branche ENTIERE — Mags,
    // eBook et Comics reunis ». On demande donc la seule branche Mags.
    //
    // YGG ET GEMINI N'Y SONT PAS, ET CE N'EST PAS UN OUBLI. Je ne connais pas leur
    // categorie de presse, et en inventer une ferait chercher sur la mauvaise etagere en
    // silence. Une source non declaree est simplement ecartee de cette famille — c'est le
    // sens prudent, et il se corrige en une ligne le jour ou la mesure est faite.
    categories: {
      c411: [7010],
      tr4ker: [7010],
      knaben: [6000000],
      // V3X : 7000 pour toutes les familles, voir la note de la famille Livres.
      v3x: [7000],
    },
    // VRAI DEPUIS LA LISEUSE PDF. Un magazine est un PDF, et l'application sait desormais
    // l'ouvrir — c'est ce qui distingue cette famille de ce qu'elle aurait ete hier.
    lisibleEnLigne: true,
  },
];

/**
 * Cette famille compte-t-elle en TOMES ?
 *
 * NON POUR LA PRESSE, et l'oublier produit des tomes inventes. Le parseur de release lit
 * un numero la ou il en voit un : « Le Journal de Spirou N°4611 26 Aout 2026 » devient le
 * tome 26 — le quantieme du mois — et « Le Monde - 1 Septembre 2026 » le tome 1. L'ecran
 * annonce alors « 1 tomes, de 26 a 26 », et l'etagere se remplit de tomes qui n'existent
 * pas. Signale a l'usage.
 *
 * Un periodique se repere par sa DATE, que le titre porte deja. Le kiosque, lui, la lit
 * proprement — voir `presse/titre-date.ts`.
 */
export function compteEnTomes(f: Famille): boolean {
  return f.id !== 'presse';
}
