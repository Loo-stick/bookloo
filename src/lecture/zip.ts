...
export interface ContenuZip {
  /** TOUS les noms d'entrees : c'est ce qui permet de reconnaitre un EPUB d'un CBZ. */
  noms: string[];
 ...
 */
/**
 * UN EPUB EST UN ZIP, et rien dans ses premiers octets ne l'en distingue.
 *
 * Ce sont ses ENTREES qui le disent : la specification impose une entree `mimetype` en
 * tete, et un `META-INF/container.xml`. Sans ce test, un roman s'ouvrait dans la
 * visionneuse, qui y trouvait la couverture et une illustration et annoncait « 2
 * planches » pour quatre cents pages — defaut deja constate sur un EPUB de tracker, et
 * revenu par une autre porte le 2026-09-07.
 */
export function estEpub(noms: readonly string[]): boolean {
  const bas = noms.map((n) => n.toLowerCase());
  return bas.includes('mimetype') || bas.some((n) => n === 'meta-inf/container.xml');
}
