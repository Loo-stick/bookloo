...
async function entreesRar4(
  lire: LecteurPlage, taille: number, avancee?: Avancee, arretSiImageCompressee = false,
): Promise<EntreeRar[]> {
  const out: EntreeRar[] = [];
  ...
  for (let n = 0; n < MAX_ENTREES && pos + 7 <= taille; n += 1) {
    ...
    if (type === 0x74 && base.length >= 32) {
      ...
      const stockee = methode === 0x30;
      out.push({ nom, stockee, taille: tailleReelle, offsetDonnees: pos + tailleEntete });
      // UNE IMAGE COMPRESSEE SUFFIT A TOUT REFUSER — `sommaireRar` le dit juste en dessous —
      // et ce refus est deja connu ici. Mesure du 2026-09-16 sur une edition Comics Tracker
      // de 228 Mo lue a distance : la conclusion tombait apres 149 SECONDES de parcours,
      // pour une reponse acquise a la premiere planche.
      if (arretSiImageCompressee && !stockee && estImage(nom)) break;
    }
...
async function entreesRar5(
  ...
  for (let n = 0; n < MAX_ENTREES && pos + 8 <= taille; n += 1) {
    ...
    if (type.valeur === 2) {
      ...
      // Meme arret qu'en RAR4, et pour la meme mesure.
      if (arretSiImageCompressee && !stockee && estImage(nom)) break;
    }
...
export async function entreesRar(
  ...
  /**
   * Arrete le parcours des la premiere IMAGE compressee.
   *
   * Reserve a `sommaireRar`, qui refuse alors l'archive entiere : continuer ne changerait
   * pas sa reponse et coutait deux minutes et demie sur un album lu a distance.
   */
  arretSiImageCompressee = false,
): Promise<EntreeRar[]> {
  const format = formatRar(await lire(0, 7));
  if (!format) throw new ErreurRar('ce fichier n est pas une archive RAR', 'pas-un-rar');
  return format === 'rar5'
    ? entreesRar5(lire, taille, avancee, arretSiImageCompressee)
    : entreesRar4(lire, taille, avancee, arretSiImageCompressee);
}
...
/**
 * Ce qu'on peut dire d'un RAR en DEUX OU TROIS LECTURES, avant de proposer « Lire ».
 *
 * POURQUOI : une archive RAR compressee ne se lit pas en ligne — la decompresser
 * demanderait de charger le tome entier en memoire, ce que ce serveur ne fait pas. La
 * sonde annoncait pourtant « lisible », et le bouton echouait neuf secondes plus tard.
 * Le parcours s'arretant a la premiere planche compressee, la reponse coute desormais
 * presque rien : autant la donner avant le clic.
 */
export async function verdictRar(
  lire: LecteurPlage, taille: number,
): Promise<{ format: 'rar'; lisible: boolean; motif?: string }> {
  try {
    const entrees = await entreesRar(lire, taille, undefined, true);
    const images = entrees.filter((e) => estImage(e.nom));
    if (images.length === 0 && entrees.length > 0) {
      return { format: 'rar', lisible: false, motif: 'cette archive RAR ne contient aucune image' };
    }
    if (images.some((e) => !e.stockee)) {
      return {
        format: 'rar',
        lisible: false,
        motif: 'cette archive RAR est compressee : il faudrait la telecharger pour la lire',
      };
    }
    return { format: 'rar', lisible: true, motif: 'archive RAR — sa preparation demande une minute environ' };
  } catch (e) {
    // Une anomalie de format se dit, elle ne se devine pas : le refus porte sa raison.
    return { format: 'rar', lisible: false, motif: (e as Error).message };
  }
}
...
export async function sommaireRar(
  ...
  const entrees = (await entreesRar(lire, taille, avancee, true))
