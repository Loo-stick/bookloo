 ...
 */
async function entreesRar4(lire: LecteurPlage, taille: number): Promise<EntreeRar[]> {
  const out: EntreeRar[] = [];
  let pos = MARQUEUR_RAR4.length;

  for (let n = 0; n < MAX_ENTREES && pos + 7 <= taille; n += 1) {
    const base = await lire(pos, Math.min(pos + MAX_ENTETE - 1, taille - 1));
    ...
    // 0x8000 : le bloc est suivi de donnees, dont la taille est le champ suivant.
    const tailleDonnees = (drapeaux & 0x8000) !== 0 && base.length >= 11
      ? base.readUInt32LE(7)
      : 0;
    ...
    if (type === 0x74 && base.length >= 32) {
      ...
      const tailleReelle = base.readUInt32LE(11);
 ...
 */
async function entreesRar5(lire: LecteurPlage, taille: number): Promise<EntreeRar[]> {
  const out: EntreeRar[] = [];
  let pos = MARQUEUR_RAR5.length;

  for (let n = 0; n < MAX_ENTREES && pos + 8 <= taille; n += 1) {
    const buf = await lire(pos, Math.min(pos + MAX_ENTETE - 1, taille - 1));
 ...
 */
export async function entreesRar(lire: LecteurPlage, taille: number): Promise<EntreeRar[]> {
  const format = formatRar(await lire(0, 7));
  if (!format) throw new ErreurRar('ce fichier n est pas une archive RAR', 'pas-un-rar');
  return format === 'rar5' ? entreesRar5(lire, taille) : entreesRar4(lire, taille);
}
 ...
 */
export async function sommaireRar(lire: LecteurPlage, taille: number): Promise<SommaireCbz> {
  const entrees = (await entreesRar(lire, taille))
