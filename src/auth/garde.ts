...
import { egalConstant } from './crypto';
...
export function exigerCsrf(req: Request, res: Response, next: NextFunction): void {
  ...
  // A TEMPS CONSTANT. Le jeton est un HMAC-SHA256, donc le gain d'une comparaison paresseuse
  // est infime — mais `egalConstant` existe deja dans ce projet, et se priver d'une garantie
  // qu'on possede n'a aucune contrepartie.
  if (!egalConstant(Buffer.from(fourni), Buffer.from(jetonCsrf(req.session.id)))) {
...
/**
 * UNE LECTURE QUI AGIT SE PROTEGE COMME UNE ECRITURE.
 *
 * « LIRE N'EST PAS TOUJOURS SANS EFFET » : la lecon est ecrite dans `routes/debrid.ts`,
 * le 2026-08-29, quand `/api/release/:hash/fichiers` est passee en POST + CSRF. Elle
 * disait deja « elle vaut au-dela de cette route ». Elle n'y a pas ete portee.
 *
 * Les surfaces de lecture — `/api/lecture`, `/api/pdf`, `/api/epub`, le telechargement
 * Z-Library — sont des GET, et elles DEBRIDENT : elles depensent le quota du compte de
 * l'utilisateur chez AllDebrid ou Z-Library. Le cookie etant en `SameSite=Lax`, une
 * navigation de premier niveau depuis une page piegee l'emporte : un lien suffisait.
 *
 * ON NE PEUT PAS LES PASSER EN POST : pdf.js demande ses plages en GET, une balise
 * `<img>` ne poste pas, et un telechargement est un `href`. Le jeton CSRF ne s'y place
 * donc pas. `Sec-Fetch-Site` repond exactement a la question posee — d'ou vient cette
 * requete — et le navigateur le pose lui-meme, hors de portee de la page attaquante.
 *
 * `same-origin` : depuis nos propres pages. `none` : saisie directe, favori, ouverture
 * d'un fichier. Tout le reste — `cross-site`, `same-site` — est refuse.
 *
 * FERME PAR DEFAUT : un en-tete ABSENT est refuse. Tous les navigateurs vises le posent
 * depuis 2020 ; l'accepter par defaut rendrait la garde inutile, puisqu'il suffirait de
 * ne pas l'envoyer.
 */
export function exigerMemeOrigine(req: Request, res: Response, next: NextFunction): void {
  const provenance = req.get('Sec-Fetch-Site');
  if (provenance === 'same-origin' || provenance === 'none') return next();
  res.status(403).json({
    erreur: 'cette lecture ne peut etre demandee que depuis Book Loo Store',
  });
}
...
/**
 * Les couvertures : genereux, mais borne.
 *
 * Chaque relais lance un process externe qui peut vivre douze secondes. Cent vingt par
 * minute couvrent largement une page de resultats et une etagere ; au-dela, c'est une
 * boucle, pas un usage. Les autres routes couteuses etaient limitees, celle-ci ne l'etait
 * pas — signale par une revue de code le 2026-08-29.
 */
export const limiteurCouverture = rateLimit({
  windowMs: 60 * 1000,
  limit: 120,
  standardHeaders: true,
  legacyHeaders: false,
  message: { erreur: 'trop de couvertures demandees, patientez un instant' },
});
