...
export function clesDeRecherche(cles: ClesEnClair) {
  return {
    ...
    // Zone-ebook ne CHERCHE que pour ses membres : sans ces deux cles, le fan-out l'ecarte
    // proprement (« cle absente »), comme un tracker sans passkey.
    zone_login: cles.zone_login, zone_mdp: cles.zone_mdp,
    // Comics Tracker de meme. OUBLIER UNE CLE ICI EST INVISIBLE A L'ENREGISTREMENT : elle
    // est bien en base, bien chiffree, et l'ecran du compte la dit « enregistree » — mais
    // le fan-out ne la voit pas et affiche « cle absente » sur un compte parfaitement
    // configure. Constate a l'usage le 2026-09-16. Le test l'exige desormais pour CHAQUE
    // source qui declare une `cleRequise`.
    ct_email: cles.ct_email, ct_mdp: cles.ct_mdp,
  };
}
