...
/**
 * Une carte par PAGE, pas une par notice.
 *
 * POURQUOI. La page d'une oeuvre cherche dans les sources avec ses `formes` — voir
 * `requeteDepuisOeuvre`. Or la BnF decrit un meme journal en plusieurs notices, que les
 * sources ne distinguent pas. Mesure du 2026-09-14 sur « Le Parisien » : trois cartes
 * intitulees « Le Parisien », qui sont en realite l'edition 77 (quotidien), « Le Parisien
 * Jeux » (trimestriel) et le supplement week-end (hebdomadaire) — et qui cherchent toutes
 * « Le Parisien ». Trois cartes pour une seule page : « je clique sur n'importe lequel,
 * j'arrive au meme endroit », signale a l'usage.
 *
 * Afficher l'edition sur chaque carte n'aurait rien arrange : elles meneraient toujours au
 * meme endroit, et feraient croire a un choix qui n'existe pas.
 *
 * LA REGLE. Les notices qui menent a la meme recherche — meme forme, a la casse et aux
 * accents pres — ne font qu'une carte, a la place de la premiere. Elle montre de preference
 * une notice au titre sans qualificatif (« Le Monde » plutot que « Le Monde (En ligne) »),
 * garde les titres de toutes pour le rapprochement, et PERD SON ANNEE : celle d'une notice
 * date une edition particuliere — 2016, 2017, 2018 pour ces trois-la — et non le journal,
 * qui parait depuis 1944.
 *
 * `parseBnf` reste inchange : il decrit ce que la BnF rend. La fusion est un choix
 * d'affichage, applique ici.
 */
export function fusionnerMemeRecherche(oeuvres: readonly Oeuvre[]): Oeuvre[] {
  const out: Oeuvre[] = [];
  const parCle = new Map<string, Oeuvre>();
  for (const o of oeuvres) {
    const cle = normaliserTitre(o.formes[0] ?? o.titre);
    // Une notice dont la forme ne se normalise en rien ne se fusionne avec rien.
    const deja = cle ? parCle.get(cle) : undefined;
    if (!deja) {
      const carte: Oeuvre = { ...o, titres: [...o.titres] };
      if (cle) parCle.set(cle, carte);
      out.push(carte);
      continue;
    }
    for (const t of o.titres) if (!deja.titres.includes(t)) deja.titres.push(t);
    delete deja.annee;
    // Un titre sans qualificatif remplace un titre qualifie, en gardant la place.
    const propre = (x: Oeuvre) => x.titre === (x.formes[0] ?? x.titre);
    if (!propre(deja) && propre(o)) {
      deja.id = o.id;
      deja.titre = o.titre;
      deja.auteur = o.auteur;
      deja.titres = [o.titre, ...deja.titres.filter((t) => t !== o.titre)];
    }
  }
  return out;
}
...
export async function chercherBnf(
  ...
  // Une carte par page : voir `fusionnerMemeRecherche`.
  return fusionnerMemeRecherche(parseBnf(await interroger(requete, signal)));
}
