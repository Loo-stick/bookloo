...
export async function chercherOeuvres(q: string, signal?: AbortSignal): Promise<Oeuvre[]> {
  ...
  // `null` signale un ECHEC, jamais un resultat vide. Sans cette distinction l'interface
  // ecrivait « MangaDex ne connait pas ce titre » quand MangaDex ne repondait pas.
  if (corps === null) throw new Error('MangaDex n a pas repondu');
