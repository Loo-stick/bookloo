 ...
 *
 * LEVE sur un echec, rend une liste VIDE quand Booknode a repondu sans connaitre le
 * titre. La bascule vers Open Library ne change pas — `premierQuiRepond` intercepte —
 * mais elle ne fait plus passer un Cloudflare ferme pour un titre introuvable.
 */
export async function chercherBooknode(
  ...
  if (mode === 'auteur') {
    ...
    if (html === null) throw new Error('Booknode n a pas repondu');
    if (pageIntrouvable(html)) return [];
