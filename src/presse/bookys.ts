...
// LE DOMAINE VIENT DES REGLAGES, comme dans `sources/ddl/bookys.ts` — et il n'y a plus
// qu'un endroit ou le corriger. Ecrit en dur ici AUSSI, « www7 » compris, il aurait
// diverge de son jumeau au premier changement de domaine.
...
export function urlListe(rubrique: Rubrique, page = 1): string {
  return page <= 1 ? `${BASE_()}/${rubrique}/new` : `${BASE_()}/${rubrique}/new?page=${page}`;
}
...
export const bookys: Kiosque = {
  ...
  async chercher(terme, _ctx, signal) {
    ...
    // sans rapport.
    const url = `${BASE_()}/search?q=${encodeURIComponent(terme)}`;
...
export async function numerosDeCategorie(
  ...
  const html = await page(`${BASE_()}/${rubrique}/${slug}`, `presse:bookys:${rubrique}:${slug}`, signal);
  if (html === null) throw new Error('bookys n a pas repondu');
