...
/**
 * LE DOMAINE VIENT DES REGLAGES, ET IL N'EST ECRIT QU'ICI.
 *
 * Il l'etait DEUX FOIS — ici et dans `sources/ddl/zone-ebook.ts` — et en dur : changer le
 * domaine demandait un rebuild, et n'en corrigeait qu'un des deux. Le site, lui, en change
 * seul.
 */
export function baseZoneEbook(): string {
  return getSettings().ddl.zoneEbook.base.replace(/\/+$/, '');
}
...
export function parseListeZoneEbook(html: string, rubrique: Rubrique): Numero[] {
  ...
  const motif = new RegExp(
    `<a[^>]+href="${baseZoneEbook()}/${rubrique}/(\\d+)-[^"]*"[^>]*>([\\s\\S]*?)</a>`,
    'g',
  );
...
export function urlRubrique(rubrique: Rubrique, page = 1): string {
  return page <= 1 ? `${baseZoneEbook()}/${rubrique}/` : `${baseZoneEbook()}/${rubrique}/page/${page}/`;
}
  ...
  async chercher(terme, ctx, signal) {
    ...
    const corps = await postForm(
      ...
      },
      {
        ...
        signal,
        maxBytes: MAX_OCTETS,
        headers: { Cookie: cookie, Referer: `${baseZoneEbook()}/` },
      },
    );
...
export async function ouvrirFicheZoneEbook(
  ...
  const html = await page(`${baseZoneEbook()}/journaux/${id}-x.html`, `presse:zone:fiche:${id}`, signal);
