...
export async function seederRelease(cfg: ConfigRustatio, torrent: Buffer, hnr: ConfigHnr): Promise<ResultatSeed> {
  // L'ADRESSE EST CELLE DE L'UTILISATEUR, VERIFIEE AVANT DE PARTIR.
  //
  // `urlRustatioValide` ne controle que la forme. Ce champ est saisi par n'importe quel
  // compte, et c'est le seul du projet a atteindre la position d'HOTE d'un appel sortant :
  // sans cette ligne, une URL interne ferait de ce serveur un relais vers son propre
  // reseau — sept appels, dont un televersement de fichier et un PATCH.
  //
  // La garde existait deja pour les `.torrent` ; elle n'avait jamais ete portee ici.
  //
  // ELLE REND UN ECHEC NOMME, ELLE NE JETTE PAS : cette fonction est appelee pendant un
  // depot, et une exception y ferait tomber le depot lui-meme — une adresse mal saisie
  // n'a pas a annuler l'action que l'utilisateur a demandee.
  try {
    await verifierUrlSortante(cfg.url);
  } catch {
    return { statut: 'echec', raison: 'adresse-rustatio-refusee' };
  }
  const h = entetes(cfg);
