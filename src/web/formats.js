...
/**
 * Ce fichier va-t-il dans la LISEUSE PDF ?
 *
 * DEFAUT REEL, signale a l'usage : un PDF trouve dans un torrent — « Le Journal de Spirou
 * - 26 Aout 2026.pdf » — partait dans la visionneuse, qui l'ouvrait comme une archive.
 * Elle n'y trouvait pas de repertoire central et le serveur repondait « ce fichier n'est
 * pas lisible en ligne, telechargez-le » — sur un fichier que l'application sait pourtant
 * lire depuis que la liseuse PDF existe.
 *
 * La branche avait ete cablee pour les releases en LIEN DIRECT seulement. Une capacite
 * ajoutee laisse derriere elle autant de chemins non branches qu'il y en avait.
 */
function estPdf(nom) {
  return /\.pdf$/i.test(String(nom || '').trim());
}
...
function raisonNonLisible(format) {
  ...
  // LE PDF N'EST PLUS « NON LISIBLE » : il a sa propre surface depuis le 2026-09-01, la
  // liseuse PDF. Meme correction que pour l'epub et l'audio avant lui, et pour la meme
  // raison — ce message ne connaissait que la visionneuse.
  if (f === 'pdf') return null;
...
if (typeof module !== 'undefined') {
  module.exports = {
    FORMATS_LISIBLES, lisibleEnLigne, raisonNonLisible, estPdf, debitAnnonce, dureeEstimee,
  };
}
