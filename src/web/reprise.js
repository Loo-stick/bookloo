...
/**
 * La NATURE d'un ouvrage : « epub » ou « audio », lue sur une position deja prise.
 *
 * Elle sert a former l'identite de depart quand la release choisie n'a jamais ete
 * ouverte — il n'y a alors aucune position a reprendre, mais il faut bien savoir OU
 * ouvrir.
 *
 * « epub » etait ecrit en dur a cet endroit-la. Un livre AUDIO est un ouvrage comme un
 * autre pour qui compte les chapitres, mais il ne s'ouvre pas au meme endroit :
 * « Commencer » sur « Ne jamais trembler », un MP3, formait `epub:<release>`, partait
 * chercher un .epub dans une release sonore, et repondait « cette release ne contient
 * plus de fichier epub » sur un livre parfaitement ecoutable. Signale a l'usage.
 *
 * SANS AUCUNE POSITION D'OUVRAGE, ON NE DEVINE PAS : la fonction rend `null`, et
 * l'appelant retombe sur ce qu'il faisait — un tome numerote.
 */
function prefixeOuvrage(positions) {
  const p = (positions || []).find(
    (x) => String(x.tome || '').startsWith('epub:') || String(x.tome || '').startsWith('audio:'),
  );
  if (!p) return null;
  return String(p.tome).slice(0, String(p.tome).indexOf(':'));
}
...
if (typeof module !== 'undefined') {
  module.exports = {
    ...
    identiteTome, fusionnerParNature, prefixeOuvrage,
  };
}
