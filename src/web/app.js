...
/**
 * Boutons d'un resultat 1001ebooks : SON ADRESSE, et rien d'autre.
 *
 * Cette source ne livre pas le fichier — il est derriere un guichet, sur un domaine
 * compagnon, avec un quota gratuit et une offre payante. Proposer « Lire » ou
 * « Telecharger » promettrait ce qu'on ne sait pas tenir. On ouvre donc la page, et les
 * octets vont du site au navigateur SANS passer par ce serveur : exactement le choix fait
 * pour « Ouvrir dans Telegram », et pour la meme raison.
 */
function sorties1001ebooks(candidat) {
  if (!candidat.lienExterne) return [];
  const a = elem('a', 'bouton-lien', 'Voir sur 1001ebooks');
  a.href = candidat.lienExterne;
  a.target = '_blank';
  a.rel = 'noopener noreferrer';
  a.title = 'Ouvre la fiche du site. Le telechargement s y fait avec votre compte, '
    + 'et le fichier ne transite pas par ce serveur.';
  return [a];
}
...
function carteRelease(candidat, etats, colonnes) {
  ...
  // ELLE A LA FORME D'UN LIEN DIRECT SANS EN ETRE UN. `ddl:` fait normalement proposer
  // « Ouvrir », qui sonde des hebergeurs pour les confier a un debrideur ; ici il n'y a
  // qu'une page publique a ouvrir soi-meme. La distinguer evite d'annoncer une impasse
  // — « aucun hebergeur joignable par vos debrideurs » — la ou tout va bien.
  const est1001 = identite.startsWith('ddl:1001ebooks:');
  ...
  const estDirect = !candidat.hash && Boolean(identite)
    && !estTelegram && !estZlibrary && !sansEmpreinte && !est1001;
  ...
  if (estTelegram || estZlibrary) {
    ...
    info.append(elem('span', 'pastille'));
    info.append(elem('span', null, 'sur le site'));
    info.title = 'Le telechargement se fait sur 1001ebooks, avec votre compte. Aucun '
      + 'debrideur n intervient, et le fichier ne passe pas par ce serveur.';
    colonneEtats.append(info);
  } else if (estDirect) {
    const info = elem('span', 'etat etat--inconnu');
    info.append(elem('span', 'pastille'));
  ...
  if (!sansEmpreinte && (hash || candidat.identite)) {
    if (est1001) {
      actions.append(...sorties1001ebooks(candidat));
    } else if (!familleLisible) {
        ...
        try {
          await api('/api/bibliotheque/suivre', {
            ...
            body: JSON.stringify({
              ...
              famille: familleCourante,
              // Le dernier tome que le catalogue annonce, quand il le sait. Il borne la
              // grille des tomes de l'etagere, qui sans lui s'arrete au plus grand tome
              // deja connu — jamais de pastille inventee.
              dernierTome: oeuvreCourante.dernierTome,
            }),
          });
...
async function deposerRelease(candidat, service, sortie, bouton) {
  ...
  try {
    ...
    const ns = noteSeed(r); // verdict seed anti-H&R (le depot est le geste qui peut le declencher)
    if (ns) noeuds.push(ns);
    sortie.replaceChildren(...noeuds);
  } catch (e) {
    bouton.disabled = false;
    bouton.textContent = libelle;
...
/**
 * Interroger les DEUX debrideurs quand l'etat n'est pas encore connu.
 *
 * Les pastilles se chargent en differe : cliquer vite laissait l'etat a « inconnu », et
 * traiter « inconnu » comme « absent » aurait fait lire le .torrent d'une release
 * pourtant en cache.
 *
 * AUCUN de ces appels ne touche le tracker. TorBox repond par une consultation en
 * lecture seule ; AllDebrid depose l'EMPREINTE une seconde puis la retire, ce qui ne
 * demande aucun annonceur et donc aucun passkey.
 */
async function trouverServiceEnCache(candidat, etats) {
  const dejaVu = ['alldebrid', 'torbox']
    .find((nom) => etats?.[nom]?.[candidat.hash] === 'en-cache');
  if (dejaVu) return dejaVu;

  try {
    const r = await api('/api/etats-cache', {
      method: 'POST', body: JSON.stringify({ hashes: [candidat.hash] }),
    });
    Object.assign(etats.torbox, r.etats?.torbox || {});
    Object.assign(etats.alldebrid, r.etats?.alldebrid || {});
    const s1 = serviceEnCache(r, candidat.hash);
    if (s1) return s1;
    // AllDebrid n'a pas de consultation en lecture seule : s'il reste inconnu, il faut
    // le lui demander explicitement. Le depot de verification est retire derriere.
    if (allDebridResteAVerifier(r, candidat.hash)) {
      const v = await api('/api/debrid/verifier', {
        method: 'POST', body: JSON.stringify({ hash: candidat.hash, service: 'alldebrid' }),
      });
      etats.alldebrid[candidat.hash] = v.etat;
      if (v.etat === 'en-cache') return 'alldebrid';
    }
  } catch {
    /* On ne sait pas : on le dira, plutot que d'aller chercher le .torrent. */
  }
  return null;
}

/**
 * Rien a lister : la release n'est chez aucun debrideur.
 *
 * ON NE LIT JAMAIS LE .torrent ICI. Le telecharger enregistrerait un download sur le
 * tracker au nom du passkey, avec l'obligation de seed qui va avec — et « Ouvrir » n'est
 * pas un geste par lequel on accepte ca. Seul « Deposer » l'est.
 */
function proposerDepot(candidat, zone, etats) {
  zone.replaceChildren();
  zone.append(elem('div', 'message',
    'Aucun debrideur n a cette release en cache. Lister ses tomes demanderait de la '
    + 'deposer chez l un d eux — c est vous qui decidez.'));
  const choix = elem('div', 'puces');
  for (const service of ['alldebrid', 'torbox']) {
    const b = elem('button', 'discret', `Deposer chez ${service}`);
    b.title = service === 'alldebrid'
      ? 'Le depot reste sur votre compte, et le telechargement peut prendre plusieurs minutes.'
      : 'TorBox telecharge la release de son cote. Comptez quelques minutes.';
    b.addEventListener('click', () => deposerRelease(candidat, service, zone, b));
    choix.append(b);
  }
  zone.append(choix);
}
...
async function ouvrirRelease(candidat, zone, bouton, etats) {
  ...
  // LE DEBRIDEUR D'ABORD, LE .torrent EN DERNIER RECOURS.
  //
  // Les deux rendent la meme liste de fichiers. Mais lire le .torrent le TELECHARGE chez
  // le tracker, ce qui y enregistre un download au nom du passkey — signale a l'usage :
  // « quand je fais Ouvrir quelque chose en cache je vois Lecture du .torrent, et c'est
  // de la que ca vient ». Le debrideur, lui, repond gratuitement des lors qu'il detient
  // la release, ce qui est justement le cas quand elle est en cache.
  zone.replaceChildren(elem('div', 'vide',
    candidat.hash ? 'verification du cache…' : 'ouverture de la fiche et sondage de l archive…'));
  const enCache = candidat.hash ? await trouverServiceEnCache(candidat, etats) : null;
  if (enCache) zone.replaceChildren(elem('div', 'vide', `liste des fichiers via ${enCache}…`));
  try {
    let structure;
    try {
      if (enCache) {
        structure = await api(`/api/release/${candidat.hash}/fichiers-debrid`, {
          method: 'POST', body: JSON.stringify({ service: enCache }),
        });
      } else if (candidat.hash) {
        proposerDepot(candidat, zone, etats);
        return;
      } else {
        structure = await api(
          // LE TITRE PART AVEC : c'est lui qui nomme le fichier quand l'hebergeur ne le
          // nomme pas, et c'est le SERVEUR qui en tire l'identite du tome.
          //
          // ET LE RAYON AVEC LUI, parce qu'il decide de la FORME de cette identite : un
          // periodique n'a pas de tome, et « Mad Movies 406 2026.pdf » entrait sous
          // `t406`. Le serveur ne peut pas le deviner depuis une identite de release.
          `/api/release/${encodeURIComponent(candidat.identite)}/fichiers`
            + `?titre=${encodeURIComponent(candidat.titre || '')}`
  ...
  function ligneTome(f) {
    ...
    if (etatDe(service) === 'en-cache') {
      ...
      lire.addEventListener('click', () => {
        ...
        // UN PDF VA DANS LA LISEUSE PDF, PAS DANS LA VISIONNEUSE. Celle-ci l'ouvrait comme
        // une archive, n'y trouvait pas de repertoire central, et le serveur repondait
        // « ce fichier n'est pas lisible en ligne » — sur un fichier parfaitement lisible.
        // Meme raison que pour l'audio deux branches plus haut : le format decide de la
        // surface.
        if (estPdf(f.chemin)) {
          // LE MEME CONTEXTE QUE LA VISIONNEUSE. Sans lui, lire un PDF n'entrait pas dans
          // la bibliotheque : la surface n'avait ni oeuvre ni tome a enregistrer, et un
          // numero lu en entier n'apparaissait nulle part. `tomeId` vient du SERVEUR — un
          // PDF et un CBZ de la meme release sont deux FICHIERS de cette release, et leur
          // inventer deux identites ferait compter le meme tome deux fois.
          window.LiseusePdf.ouvrir({
            ...
            oeuvre: oeuvreSuivie(),
            tome: f.tomeId,
            reprise: repriseConnue(f.tomeId),
            titreOeuvre: oeuvreCourante?.titre,
            couverture: oeuvreCourante?.couverture,
          });
          return;
...
function rendreHebergeurs(candidat, structure, zone) {
  ...
  // de le deviner.
  if (archive.lisible) {
    // LE NOMBRE DE PLANCHES N'EST PAS TOUJOURS CONNU. Il l'est pour un ZIP, dont le
    // repertoire central se lit d'un coup. Il ne l'est ni pour un RAR — ses en-tetes se
    // parcourent un par un, et les compter couterait une minute — ni pour un PDF, qui n'a
    // pas de planches du tout. « undefined planches » s'affichait alors, signale a l'usage.
    info.append(elem('div', 'tranche-legende', archive.format === 'pdf'
      ? 'document PDF — lisible en ligne'
      : typeof archive.nbPlanches === 'number'
        ? `archive lisible en ligne — ${archive.nbPlanches} planches`
        : 'archive lisible en ligne — sa preparation demande un moment'));
  } else {
  ...
  let service = ['alldebrid', 'torbox']
    .find((s) => structure.hebergeurs.some((h) => h.supporte[s] && !refuse(h, s))) || 'alldebrid';

  // UN REFUS APPARTIENT AU SERVICE QUI L'A PRONONCE. Ce qu'AllDebrid ne sait pas ouvrir,
  // TorBox le peut peut-etre : compter le refus de l'un contre l'autre eteignait un
  // debrideur parfaitement capable.
  function refuse(h, s) { return (h.refusePar || []).includes(s); }

  function majChoix() {
    choix.replaceChildren();
    // AUCUN DEBRIDEUR N'EST REQUIS : il n'y a rien a choisir. Proposer « alldebrid » et
    // « torbox » sur une release que le site sert lui-meme laisse croire qu'il faut en
    // configurer un — alors que c'est precisement la source qui n'en demande aucun.
    if (structure.hebergeurs.every((h) => h.sansDebrideur)) return;
    for (const s of ['alldebrid', 'torbox']) {
      const n = structure.hebergeurs.filter((h) => h.supporte[s] && !refuse(h, s)).length;
  ...
  function majTable() {
    corps.replaceChildren();
    const table = elem('table', 'tomes tomes--hebergeurs');
    for (const h of structure.hebergeurs) {
      const tr = elem('tr');
      const utilisable = h.supporte[service] && !refuse(h, service);
      tr.append(elem('td', 'col-tome', utilisable ? '●' : refuse(h, service) ? '◌' : '○'));
      tr.append(elem('td', 'col-nom', h.hebergeur));
      tr.append(elem('td', 'col-taille', poids(h.taille)));

      const actions = elem('td', 'col-actions');
      // La decision vit dans `actions-release.js`, module pur : elle avait derive ici,
      // « Lire » consultant la sonde et « Telecharger » non.
      const quoi = actionsRelease(h, service, archive);
      if (quoi.etat !== 'lire' && quoi.etat !== 'telecharger') {
        // Grise, PAS cache : le cacher ferait croire que l'hebergeur n'existe pas.
        actions.append(elem('span', 'puce', quoi.motif));
        // LA DERNIERE SORTIE, QUAND AUCUN DEBRIDEUR NE PEUT SERVIR.
        //
        // Le fichier existe et le navigateur l'ouvre — verifie le 2026-09-02 sur un lien
        // dailyuploads qu'AllDebrid et TorBox refusent tous les deux. Sans ce lien,
        // l'ecran expliquait l'impasse sans en proposer la sortie.
        //
        // C'est l'adresse PUBLIQUE de la fiche, pas un lien debride : rien du compte de
        // l'utilisateur n'y circule.
        if (h.url) {
          // UN BOUTON, PAS UN LIEN NU. Toutes les autres actions de cette colonne en sont ;
          // un `<a>` n'herite pas de `button.discret` et se lisait comme un corps etranger.
          const dehors = elem('button', 'discret', 'Chez l hebergeur');
          dehors.type = 'button';
          dehors.title = 'Ouvre la page de l hebergeur dans un nouvel onglet, hors debrideur';
          // `noopener` : la page ouverte ne doit pas pouvoir manipuler celle-ci.
          // `noreferrer` : l'hebergeur n'a pas a savoir d'ou vient la visite.
          dehors.addEventListener('click', () => window.open(h.url, '_blank', 'noopener,noreferrer'));
          actions.append(dehors);
        }
      } else {
        ...
        if (quoi.etat === 'lire') {
          ...
          lire.addEventListener('click', async () => {
            ...
            // LIBGEN SE LIT DEPUIS SA COPIE LOCALE : son CDN est trop lent pour une lecture a
            // distance. On attend le prechargement, barre a l'appui, puis on ouvre.
            if (h.hebergeur === 'libgen') {
              lire.disabled = true;
              try {
                if (!(await window.Precharge.attendre({ identite: candidat.identite, zone: sortie, api }))) return;
              } catch (e) {
                afficherErreur(sortie, e.message);
                return;
              } finally {
            ...
            // TROIS SURFACES, ET LE FORMAT CHOISIT. La visionneuse affiche des planches
            // numerotees, la liseuse EPUB un flux de texte, la liseuse PDF des pages
            // dessinees. Envoyer un PDF a la visionneuse ouvrait une surcouche vide.
            if (archive.format === 'pdf') {
              window.LiseusePdf.ouvrir({
                ...
                oeuvre: oeuvreSuivie(),
                tome: h.tomeId,
                reprise: repriseConnue(h.tomeId),
                // CE QUE CETTE RELEASE SERT, meme quand c'est un seul tome. Sans cette
                // liste, le favori entrait avec `tomes` vide : la pastille du tome lu
                // restait en POINTILLE — « manquant » — parce qu'aucune release ne
                // declarait le servir. « J'ai toujours le tome 1 en pointille. »
                tomes: [h.tomeId],
                titreOeuvre: oeuvreCourante?.titre,
                couverture: oeuvreCourante?.couverture,
              });
            ...
            // UN EPUB A SA LISEUSE, comme un PDF a la sienne. C'est un ZIP, donc rien
            // dans ses premiers octets ne le distingue d'un CBZ : la sonde le reconnait a
            // ses entrees — `mimetype`, `META-INF/container.xml` — et le DIT. Sans cela un
            // roman s'ouvrait dans la visionneuse, qui y trouvait la couverture et une
            // illustration et annoncait « 2 planches » pour quatre cents pages.
            if (archive.format === 'epub') {
              window.Liseuse.ouvrir({
                identite: candidat.identite,
                // `service` ET `fichier` SONT OBLIGATOIRES ICI. La liseuse ne les transmet
                // que si on les lui donne, et `resoudreDemande` les exige pour toute
                // identite qui n'est ni Telegram ni Z-Library — ces deux-la se suffisent.
                // Les omettre faisait echouer l'ouverture sur « aucun fichier demande »,
                // rendu en 502 : j'avais calque la forme Z-Library, qui est justement le
                // cas ou ils n'existent pas.
                service,
                fichier: nom,
                titre: nom,
                oeuvre: oeuvreSuivie(),
                tome: h.tomeId,
                // `repriseOuvrage` ET NON `repriseConnue`. La visionneuse se contente d'un
                // NOMBRE — une planche — la liseuse attend un OBJET, parce qu'un chapitre
                // est un flux continu et qu'il faut dire ou l'on en est dedans. Passer le
                // nombre laisse `reprise.chapitre` a `undefined` : la lecture repart du
                // debut, sans la moindre erreur. Le depot le dit deja, deux fonctions plus
                // haut ; calquer la branche PDF sans le lire aurait suffi a le refaire.
                reprise: repriseOuvrage(h.tomeId),
                tomes: [h.tomeId],
                titreOeuvre: oeuvreCourante?.titre,
                famille: familleCourante,
                couverture: oeuvreCourante?.couverture,
            ...
            window.Visionneuse.ouvrir({
              hash: candidat.identite, service, fichier: nom, titre: nom,
              oeuvre: oeuvreSuivie(), tome: h.tomeId, reprise: repriseConnue(h.tomeId),
              // Idem : un lien direct sert UN fichier, et il doit le dire.
              tomes: [h.tomeId],
              titreOeuvre: oeuvreCourante?.titre,
            famille: familleCourante, mangadexId: oeuvreCourante?.mangadexId,
              couverture: oeuvreCourante?.couverture,
...
// Verdict du seed anti-H&R renvoye par /api/debrid/deposer. null si rien a annoncer.
function noteSeed(r) {
...
function carteOeuvre(o) {
  ...
  if (o.couverture) {
    ...
    // UNE VIGNETTE TYPOGRAPHIQUE, PAS UN CARRE VIDE. La BnF ne fournit aucune image — une
    // notice decrit un periodique, et ce sont ses NUMEROS qui ont des couvertures — et le
    // CDN de Booknode refuse le hotlinking. Le nom compose est la bonne representation
    // pour un journal : son identite visuelle EST sa manchette.
    const vignette = elem('div', 'sans-image');
    vignette.style.setProperty('--teinte', String(teinteDuTitre(o.titre)));
    vignette.append(elem('span', null, texteDeVignette(o.titre)));
    a.append(vignette);
  }
...
async function pageAccueil() {
  ...
  familleCourante = parametres.get('famille') || 'mangas';
  // UNE RECHERCHE PEUT ARRIVER PAR L'URL. Le kiosque de presse s'en sert : cliquer un
  // numero ouvre la recherche NORMALE sur son titre, dans la famille Presse — donc les
  // memes cartes, le meme panneau, la meme verification de cache et le meme « Lire » que
  // partout ailleurs. Le kiosque n'a pas a refaire ce chemin, il y MENE.
  const requeteUrl = (parametres.get('q') || '').trim();
  try {
  ...
  if (requeteUrl) {
    champ.value = requeteUrl;
    // `chercherCatalogue` bascule d'elle-meme en recherche libre quand la famille n'a
    // aucun catalogue — ce qui est le cas de la presse. Aucun chemin special ici non plus.
    chercherCatalogue();
    return;
  }
  champ.focus();
...
async function pageOeuvre() {
  ...
  await suivreFlux(avecFamille(`/api/oeuvre/${encodeURIComponent(id)}/releases/flux`), resultats, (o) => {
    ...
    if (o.couverture) {
      ...
      // MEME VIGNETTE QUE DANS LA GRILLE. Deux representations differentes du meme titre —
      // un carre vide ici, une manchette la — donneraient l'impression d'avoir change
      // d'oeuvre en cliquant.
      const vignette = elem('div', 'sans-image');
      vignette.style.setProperty('--teinte', String(teinteDuTitre(o.titre)));
      vignette.append(elem('span', null, texteDeVignette(o.titre)));
      bloc.append(vignette);
    }
...
async function pageCompte() {
  ...
  async function recharger() {
    ...
    for (const groupe of GROUPES) {
      ...
      for (const [nom, label, aide] of groupe.cles) {
        ...
        champ.type = 'password';
        champ.id = `cle-${nom}`;
        // « new-password », PAS « off ». Les navigateurs IGNORENT « off » sur un champ
        // `type=password` et y injectent une identite enregistree quoi qu'il arrive.
        // Signale a l'usage le 2026-08-29 : le gestionnaire de mots de passe d'un
        // utilisateur remplissait ses champs de cle, et comme on n'envoie que ce qui n'est
        // pas vide, c'est le mot de passe du navigateur qui partait au serveur — « sur
        // bookloo il save pas la bonne clee, le hash n'est pas le bon ». Son `fetch` direct
        // vers la meme route passait, lui : le formulaire etait le seul coupable.
        //
        // Aucun `name` non plus : il aide les gestionnaires a prendre ce formulaire pour un
        ...
        champ.placeholder = presence[nom]
          ...
          : aide;
        // UN SECRET QU'ON NE PEUT PAS RELIRE EST UN SECRET QU'ON NE PEUT PAS VERIFIER.
        // Sans ce bouton, rien ne distinguait « ma passkey » de « ce que le navigateur a mis
        // a sa place » : le champ est masque, et l'erreur restait invisible jusqu'a ce qu'un
        // tracker refuse la cle.
        const rangee = elem('div', 'cle-saisie');
        rangee.append(champ);
        const oeil = elem('button', 'discret', 'Afficher la cle');
        oeil.type = 'button';
        ...
        oeil.addEventListener('click', () => {
          ...
          oeil.textContent = montre ? 'Masquer la cle' : 'Afficher la cle';
    ...
    // Bloc zone-ebook : PSEUDO + mot de passe. Le pseudo revient du serveur et pre-remplit
    // le champ ; le mot de passe, jamais.
    const blocE = elem('div', 'groupe-cles');
    blocE.append(elem('h2', null, 'Zone-ebook — journaux et magazines'));
    blocE.append(elem('p', 'titres',
      'Le kiosque fonctionne sans compte : les dernieres parutions sont publiques. '
      + 'La RECHERCHE, elle, n est servie qu aux membres du site.'));

    let loginConnu = null;
    try {
      const z = await api('/api/compte/zone-ebook');
      loginConnu = z.login;
    } catch {
      // Le compte reste utilisable meme si cette lecture echoue : le champ sera vide.
    }

    const ligneE1 = elem('div', 'cle');
    const enteteE1 = elem('div', 'cle-entete');
    const labE1 = elem('label', null, 'Pseudo');
    labE1.htmlFor = 'cle-zone_login';
    enteteE1.append(labE1);
    enteteE1.append(elem('span', 'presence', loginConnu ? 'enregistre' : 'non renseigne'));
    ligneE1.append(enteteE1);
    // LE PSEUDO, PAS L ADRESSE. Mesure sur le site : se connecter avec l adresse ne pose
    // que `PHPSESSID` — aucune session. Le champ le dit, plutot que de laisser essayer.
    const aideE = elem('div', 'titres', 'Votre pseudo du site, pas votre adresse e-mail : zone-ebook refuse la connexion par adresse.');
    const champE1 = elem('input');
    champE1.type = 'text';
    champE1.id = 'cle-zone_login';
    champE1.autocomplete = 'off';
    champE1.placeholder = 'votre pseudo';
    if (loginConnu) champE1.value = loginConnu;
    ligneE1.append(champE1);
    ligneE1.append(aideE);
    blocE.append(ligneE1);

    const ligneE2 = elem('div', 'cle');
    const enteteE2 = elem('div', 'cle-entete');
    const labE2 = elem('label', null, 'Mot de passe');
    labE2.htmlFor = 'cle-zone_mdp';
    enteteE2.append(labE2);
    enteteE2.append(elem('span', 'presence', presence.zone_mdp ? 'enregistre' : 'non renseigne'));
    ligneE2.append(enteteE2);
    const champE2 = elem('input');
    champE2.type = 'password';
    champE2.id = 'cle-zone_mdp';
    champE2.autocomplete = 'new-password';
    champE2.placeholder = presence.zone_mdp
      ? 'enregistre — saisir pour remplacer'
      : 'mot de passe zone-ebook';
    ligneE2.append(champE2);
    blocE.append(ligneE2);
    liste.append(blocE);

    // Bloc Comics Tracker : L'ADRESSE, et non un pseudo. Son authentification est Firebase,
    // qui ne connait que l'e-mail — mesure du 2026-09-16 sur sa configuration publique.
    // L'adresse revient du serveur et pre-remplit le champ ; le mot de passe, jamais.
    const blocC = elem('div', 'groupe-cles');
    blocC.append(elem('h2', null, 'Comics Tracker — comics en editions francaises'));
    blocC.append(elem('p', 'titres',
      'La recherche du site est publique. LIRE un comic demande votre compte : '
      + 'ses fichiers ne sont servis qu a ses membres.'));

    let emailCt = null;
    try {
      const c = await api('/api/compte/comics-tracker');
      emailCt = c.email;
    } catch {
      // Le compte reste utilisable meme si cette lecture echoue : le champ sera vide.
    }

    const ligneC1 = elem('div', 'cle');
    const enteteC1 = elem('div', 'cle-entete');
    const labC1 = elem('label', null, 'Adresse e-mail');
    labC1.htmlFor = 'cle-ct_email';
    enteteC1.append(labC1);
    enteteC1.append(elem('span', 'presence', emailCt ? 'enregistree' : 'non renseignee'));
    ligneC1.append(enteteC1);
    const champC1 = elem('input');
    champC1.type = 'email';
    champC1.id = 'cle-ct_email';
    champC1.autocomplete = 'off';
    champC1.placeholder = 'votre adresse Comics Tracker';
    if (emailCt) champC1.value = emailCt;
    ligneC1.append(champC1);
    blocC.append(ligneC1);

    const ligneC2 = elem('div', 'cle');
    const enteteC2 = elem('div', 'cle-entete');
    const labC2 = elem('label', null, 'Mot de passe');
    labC2.htmlFor = 'cle-ct_mdp';
    enteteC2.append(labC2);
    enteteC2.append(elem('span', 'presence', presence.ct_mdp ? 'enregistre' : 'non renseigne'));
    ligneC2.append(enteteC2);
    const champC2 = elem('input');
    champC2.type = 'password';
    champC2.id = 'cle-ct_mdp';
    champC2.autocomplete = 'new-password';
    champC2.placeholder = presence.ct_mdp
      ? 'enregistre — saisir pour remplacer'
      : 'mot de passe Comics Tracker';
    ligneC2.append(champC2);
    blocC.append(ligneC2);
    liste.append(blocC);
    ...
    const champ = (id, type, libelle, aide) => {
      ...
      i.type = type;
      i.id = id;
      // Meme regle que pour les cles : « off » est ignore sur un champ masque, et le jeton
      // Rustatio en est un.
  ...
  document.getElementById('enregistrer').addEventListener('click', async () => {
    ...
    // Z-Library et zone-ebook : deux cles independantes chacun. Un champ vide est
    // simplement omis — le mot de passe deja enregistre n'est donc pas efface quand on ne
    // corrige que l'identifiant.
    for (const nom of ['zlib_email', 'zlib_mdp', 'zone_login', 'zone_mdp', 'ct_email', 'ct_mdp']) {
      const champ = document.getElementById(`cle-${nom}`);
...
async function pageAdmin() {
  ...
  async function recharger() {
    // QUI SUIS-JE. Sert a ne PAS proposer la suppression de son propre compte : le serveur
    // la refuse deja, mais offrir un geste qu'on refusera ensuite est une promesse qu'on ne
    // tient pas. Le serveur garde sa verification — l'interface n'est jamais la garantie.
    let moi = null;
    try { ({ userId: moi } = await api('/api/moi')); } catch { moi = null; }
    ...
    for (const u of utilisateurs) {
      ...
      if (u.id !== moi) {
        const suppr = elem('button', 'discret', 'Supprimer');
        suppr.addEventListener('click', async () => {
          // IRREVERSIBLE : le CASCADE emporte les cles chiffrees, la bibliotheque et la
          // progression avec le compte. Un clic distrait sur la mauvaise ligne ne doit pas
          // suffire, d'ou une confirmation qui NOMME le compte.
          if (!confirm(`Supprimer definitivement le compte « ${u.login} » ? `
            + 'Ses cles, sa bibliotheque et sa progression sont effacees avec lui.')) return;
          suppr.disabled = true;
          try {
            await api(`/api/admin/utilisateurs/${u.id}`, { method: 'DELETE' });
            await recharger();
          } catch (e) {
            suppr.disabled = false;
            sortie.replaceChildren(elem('div', 'vide', `suppression impossible : ${e.message}`));
          }
        });
  ...
  await recharger().catch((e) => afficherErreur(sortie, e.message));
  await adressesDesSources().catch(() => {
    /* Les comptes restent utilisables meme si cette section ne charge pas. */
  });
}

/**
 * Les adresses des sources, modifiables.
 *
 * POURQUOI CET ECRAN EXISTE. Wawacity, zone-ebook, bookys et Z-Library changent de domaine
 * seuls — trois d'entre eux etaient ecrits EN DUR dans le code, et les corriger demandait
 * de reconstruire l'image. « Je n'ai rien sur mon Administration pour changer les url de
 * nos sources », signale a l'usage.
 *
 * LES CLES N'Y SONT PAS : une adresse est une donnee d'exploitation, partagee ; une
 * passkey appartient a une personne et vit chiffree par sa DEK. Les melanger ferait ecrire
 * un secret dans un fichier lisible.
 */
async function adressesDesSources() {
  const zone = document.getElementById('adresses');
  const sortie = document.getElementById('sortie-adresses');
  const bouton = document.getElementById('enregistrer-adresses');
  if (!zone || !bouton) return;

  const { adresses, zlibrary } = await api('/api/admin/adresses');
  const champs = new Map();
  zone.replaceChildren();

  const poser = (id, libelle, valeur, aide) => {
    const ligne = elem('div', 'cle');
    const entete = elem('div', 'cle-entete');
    const lab = elem('label', null, libelle);
    lab.htmlFor = `adresse-${id}`;
    entete.append(lab);
    if (aide) entete.append(elem('span', 'presence', aide));
    ligne.append(entete);
    const champ = elem('input');
    champ.type = 'text';
    champ.id = `adresse-${id}`;
    // `url`, ET NON `off`. Le projet interdit `off` — les navigateurs l'ignorent sur un
    // champ de mot de passe, ce qui a coute une matinee a un utilisateur. Ici le champ
    // porte une adresse : `url` est la valeur juste, et elle ne ment pas au navigateur.
    champ.autocomplete = 'url';
  ...
  for (const a of adresses) {
    poser(a.id, a.libelle, a.valeur, a.interneAutorise ? 'adresse interne acceptee' : '');
  }
  // Z-LIBRARY PORTE UN DOMAINE, PAS UNE URL : y coller « https://… » donnerait
  // « https://https://… » a la construction. Le champ le dit.
  poser('zlibrary', zlibrary.libelle, zlibrary.valeur, 'domaine seul, sans https://');

  bouton.addEventListener('click', async () => {
    bouton.disabled = true;
    sortie.replaceChildren(elem('div', 'vide', 'enregistrement…'));
    const corps = {};
    for (const [id, champ] of champs) corps[id] = champ.value.trim();
    try {
      const r = await api('/api/admin/adresses', { method: 'PUT', body: JSON.stringify(corps) });
      sortie.replaceChildren(elem('div', 'message',
        `${r.ecrites.length} adresse(s) enregistree(s). Elles sont deja actives.`));
    } catch (e) {
      // LE MOTIF EN ENTIER : il nomme la source refusee et la raison, et rien n'a ete
      // ecrit — un enregistrement a moitie fait laisserait un ecran dont on ne sait plus
      // ce qu'il porte.
      afficherErreur(sortie, e.message);
    } finally {
      bouton.disabled = false;
