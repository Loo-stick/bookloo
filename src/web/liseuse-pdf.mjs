...
function ouvrir({ hash, service, fichier, titre, oeuvre, titreOeuvre, couverture, famille, tome, reprise,
  ...
  function corpsPosition() {
    ...
    return {
      ...
      // CE QUE LA RELEASE SERT, ET LE DIRE EST OBLIGATOIRE. Sans cette liste le favori
      // entre avec `tomes` vide, et la pastille du numero lu reste en POINTILLE — comme
      // si aucune release ne le servait. La visionneuse transmettait deja ce champ ; les
      // trois autres surfaces le recevaient de leur appelant et le jetaient en silence,
      // ce qui fait que le correctif ecrit pour elle n'a jamais rien change ici.
      //
      // A DEFAUT, CE QUE CETTE SURFACE OUVRE : elle lit UN document, elle peut donc
      // toujours declarer le servir, et c'est vrai par construction.
      tomes: Array.isArray(tomes) && tomes.length
        ? tomes
        : [tome || `f:${fichier || titre || ''}`],
