...
function hebergementUtilisable(structure) {
  ...
  // Un lien qu'un service a refuse ne compte pas POUR CE SERVICE : le compter ferait choisir un service qui
  // echouera a l'ouverture, apres l'attente.
  const vivants = hebergeurs.filter((h) => h && !(h.refusePar || []).length);
