 ...
 */
function tomesDeLaGrille(principale, appoints, positions, dernierTome, numerote = true) {
  const connus = [
  ...
  // UNE FAMILLE QUI NE COMPTE PAS EN TOMES N'A PAS DE SUITE A DESSINER. Un numero n'est
  // pas un rang : « Mad Movies 406 » est UN magazine, pas le quatre-cent-sixieme d'une
  // collection qu'on posséderait. La grille en deduisait 406 cases, dont 405 en pointille
  // pour des numeros que personne n'a jamais eus — et le compteur annoncait « 0 / 407 ».
  // Le commentaire de l'etagere le disait deja : « LES NUMEROS NE SONT PAS DES TOMES : on
  // ne peut pas dessiner une grille de 1 a N. » La regle vit maintenant ici, ou elle est
  // executee.
  if (!numerote) {
    const seuls = [];
    const dejaVus = new Set();
    for (const t of connus) {
      if (t && !dejaVus.has(t)) { seuls.push(t); dejaVus.add(t); }
    }
    return seuls;
  }
