...
(function () {
  ...
  async function changerRelease(entree, rafraichir) {
    ...
    zone.replaceChildren(etat, cadre, cadreIncertaines);
      ...
      for (;;) {
        ...
        for (const ligne of morceaux) {
          ...
          for (const c of msg.ajouts || []) {
            ...
            const ligne = ligneRelease(c, entree, p, rafraichir, lignes);
            if (c.certain === false) {
              incertaines++;
          ...
          if (incertaines > 0) {
            blocIncertaines.hidden = false;
            resumeIncertaines.textContent =
              `${incertaines} correspondance(s) incertaine(s) — deplier`;
          }
  ...
  function ligneRelease(c, entree, p, rafraichir, lignes) {
    ...
    if (natureHorsDebrideur) {
      ...
      const choisirSeul = elem('button', 'principal', 'Choisir');
      choisirSeul.addEventListener('click', () => poserRaccourci(c, cle, entree, p, rafraichir, tdEtat, choisirSeul));
    ...
    choisir.addEventListener('click', () => poserRaccourci(c, cle, entree, p, rafraichir, tdEtat, choisir));
  ...
  async function ouvrirLienDirect(entree, tomeId, reprise, sortie, surFermeture) {
    ...
    if (issue !== 'lire') {
      ...
      sortie.append(issue === 'telecharger'
        ...
        : boutonChangerRelease(entree, surFermeture));
      return;
    }
  ...
  async function ouvrirTome(entree, tomeId, reprise, sortie, surFermeture) {
    ...
    if (voie.voie === 'visionneuse') {
      ...
      window.Visionneuse.ouvrir({
        ...
        fichier: entree.titreRelease || entree.titre,
        titre: entree.titreRelease || entree.titre,
        oeuvre: entree.oeuvre,
        tome: tomeId,
        reprise: reprise || 0,
        titreOeuvre: entree.titre,
