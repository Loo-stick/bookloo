...
(function () {
  ...
  /**
   * Sous quelle forme cette oeuvre s'ouvre-t-elle ? La regle vit dans `reprise.js`, ou
   * elle est EXECUTEE par des tests — c'est la ou vivent deja toutes les decisions de
   * reouverture, et l'ecrire ici l'aurait fait diverger comme l'etiquette avant elle.
   */

  /**
   * TOME OU NUMERO ? Le serveur repond en JOIGNANT la table des parutions : il la pose des
   * que la famille ne compte pas en tomes, meme vide. Le front ne tient donc pas de
   * seconde liste des rayons dates — `familles/table.ts` fait foi, et elle seule.
   */
  const uniteDe = (entree) => (entree && entree.parutions ? 'numero' : 'tome');

  /**
   * Les parutions DEJA LUES de cette oeuvre.
   *
   * On les prend sur les POSITIONS, jamais sur la table entiere : celle-ci couvre aussi
   * les numeros qu'une release porte sans qu'on les ait ouverts, et tout serait marque
   * « lu » des la premiere lecture.
   */
  const parutionsLues = (entree) => new Set(
    (entree.positions || [])
      .map((p) => parutionDe(p.tome, entree))
      .filter(Boolean),
  );

  /**
   * La parution d'une identite : le LIBELLE pour l'oeil, la DATE pour l'ordre.
   *
   * Les deux voyagent ensemble depuis le serveur parce que seule la date se trie :
   * « 2 septembre 2026 » passerait avant « 26 aout 2026 » dans l'ordre alphabetique.
   */
  const parutionDe = (id, entree) => (entree && entree.parutions && entree.parutions[id]
    ? entree.parutions[id].libelle
    : null);
  const dateDe = (id, entree) => (entree && entree.parutions && entree.parutions[id]
    ? entree.parutions[id].date
    : null);

  const libelleTome = (id, entree) => {
    if (estEcoute(id)) return 'le livre audio';
    if (estLivre(id)) return 'le livre';
    // UN PERIODIQUE SE NOMME PAR SA DATE, pas par un rang. La date est lue cote serveur
    // par le parseur du kiosque : un second lecteur ici aurait diverge du premier, et
    // c'est exactement la faute qui affichait « Tome 26 » pour le numero 4611 — le
    // quantieme du mois pris pour un tome.
    //
    // « du 26 aout 2026 » mais « de septembre 2026 » : un mensuel n'a pas de quantieme,
    // et le libelle commence alors par une lettre. C'est tout ce que le front decide.
    const parution = parutionDe(id, entree);
    if (parution) return `le numero ${/^\d/.test(parution) ? 'du' : 'de'} ${parution}`;
    // Sans date lisible dans son nom, un numero garde son nom : il reste reconnaissable,
    // la ou une date devinee serait fausse.
  ...
  function tomesAAfficher(entree) {
    const grille = tomesDeLaGrille(
      principaleDe(entree), entree.appoints || [], entree.positions, entree.dernierTome,
      // LA TABLE DES PARUTIONS DIT LE RAYON. Elle est presente, meme vide, pour une
      // famille qui ne compte pas en tomes — c'est deja ce qui fait dire « numeros »
      // plutot que « tomes » a l'ecran, et `parution.ts` l'ecrit noir sur blanc.
      !entree.parutions,
    );
    if (!entree.parutions) return grille;
    // LE PLUS RECENT EN TETE : un periodique se parcourt a l'envers d'une serie. Sans
    // ordre, les numeros se rangeaient dans l'ordre ou on les a rencontres — celui des
    // rattachements, qui ne veut rien dire pour qui cherche le dernier paru.
    return grille.slice().sort(
      (a, b) => String(dateDe(b, entree) || '').localeCompare(String(dateDe(a, entree) || '')),
    );
  }
  ...
  function barreAvancement(entree) {
    ...
    const unite = uniteDe(entree);
    const barre = elem('div', 'avancement');
    barre.setAttribute('role', 'img');
    barre.setAttribute('aria-label', `${lus} ${unite}${lus > 1 ? 's' : ''} lu${lus > 1 ? 's' : ''} sur ${total}`);
  ...
  const etiquette = (id, entree) => parutionDe(id, entree) || etiquetteTome(id);
  ...
  function grilleTomes(entree, surClic, surManquant) {
    ...
    for (const tomeId of tomesAAfficher(entree)) {
      ...
      const b = elem('button', 'pastille-tome', etiquette(tomeId, entree));
      b.type = 'button';
      ...
      const quoi = libelleTome(tomeId, entree);
      b.setAttribute(
        'aria-label',
  ...
  async function panneauReleases(entree, rafraichir, cible) {
    const p = window.Panneau.ouvrir({
      ...
      sousTitre: cible
        ? `Prendre ${libelleTome(cible, entree)} ailleurs`
    ...
    if (!famille) return;
    // ET ON LA RETIENT. Demandee sans etre enregistree, la question revenait a chaque
    // recherche — et l'oeuvre continuait de s'afficher en « tomes » alors qu'on venait
    // de dire qu'elle etait dans la presse. C'est aussi le seul moyen de rattraper les
    // oeuvres suivies avant que la colonne existe.
    if (!entree.famille) {
      entree.famille = famille;
      api('/api/bibliotheque/suivre', {
        method: 'POST',
        body: JSON.stringify({ oeuvre: entree.oeuvre, titre: entree.titre, famille }),
      }).catch(() => {});
    }
    ...
    /**
     * LE PLUS RECENT EN HAUT. C'est le seul ordre qui vaille pour un periodique : on
     * cherche le numero de la semaine, pas celui qu'une source a rendu en premier.
     *
     * ON TRIE SUR LA DATE BRUTE, jamais sur le libelle : « 2 septembre 2026 » passerait
     * avant « 26 aout 2026 » dans l'ordre alphabetique, soit l'inverse du vrai.
     *
     * `append` DEPLACE un noeud deja pose : reordonner ne recree aucune ligne, et les
     * etats de cache deja peints restent en place.
     */
    function trierParParution() {
      const rangs = posees.filter((x) => x.certain);
      if (!rangs.some((x) => x.c.paruLe)) return;
      rangs.sort((a, b) => String(b.c.paruLe || '').localeCompare(String(a.c.paruLe || '')));
      liste.append(...rangs.map((x) => x.ligne));
    }
  ...
  /**
   * LIRE UN NUMERO, au lieu de le poser comme raccourci de l'oeuvre.
   *
   * Un periodique n'a pas UNE release : il en a une par parution. Poser le numero du
   * 19 aout comme raccourci du « Journal de Spirou » remplacait celui du 26 — la carte
   * annoncait alors la release du 19 sous la position du 26, et le second numero
   * n'apparaissait nulle part. « y a incoherence la », signale a l'usage.
   *
   * Chaque numero devient donc un APPOINT, qui sert SON identite et elle seule. On ne
   * connait cette identite qu'apres l'inventaire du debrideur, d'ou l'ordre : cache
   * d'abord, inventaire, rattachement, lecture.
   *
   * LE CACHE PASSE TOUJOURS EN PREMIER. Contrainte permanente du projet : aucune lecture
   * ne declenche un telechargement chez le debrideur.
   */
  async function lireNumero(c, cle, entree, p, rafraichir, tdEtat, bouton) {
    bouton.disabled = true;
    try {
      const { service, chemin, format } = await ouOuvrirCeNumero(c, cle, entree, tdEtat);
      const tomeId = `f:${chemin}`;
      // L'APPOINT NE SERT QUE CE NUMERO. Lui donner la plage du titre le ferait couvrir
      // des numeros qu'il ne contient pas, et un autre numero s'ouvrirait sur ce fichier.
      await api('/api/bibliotheque/appoint', {
        method: 'POST',
        body: JSON.stringify({
          oeuvre: entree.oeuvre,
      ...
      p.fermer();
      ouvrirFichier({
        famille: entree.famille || undefined,
        hash: cle,
        service,
        fichier: chemin,
        titre: chemin.split('/').pop(),
        format,
        oeuvre: entree.oeuvre,
        tome: tomeId,
        reprise: 0,
        ...
        surFermeture: rafraichir,
      });
    } catch (e) {
      bouton.disabled = false;
      // LE MOTIF EN ENTIER. Coupe a 60 caracteres, il rendait « … aucun des 2 lien(s)
      // utilisabl » — une phrase tranchee au milieu d'un mot, qui ne dit plus rien.
      // UNE PHRASE N'EST PAS UNE PASTILLE. `.etat` est une pilule faite pour trois mots —
      // « en cache », « pas en cache ». Un motif entier dedans donnait une pilule grise
      // large comme la moitie de la ligne. Le motif est du TEXTE ; la pastille reste aux
      // etats courts.
      tdEtat.replaceChildren(elem('div', 'motif-echec', e.message));
      // ET LA SORTIE, quand la sonde a rapporte des hebergeurs. Sans elle, la ligne
      // gardait « Lire ce numero » comme seul geste — celui qui venait d'echouer.
      const dehors = e.structure ? sortiesHebergeurs(e.structure) : null;
      if (dehors) {
        // LA SORTIE SE POSE SOUS LE MOTIF, pas dans la colonne d'actions. Les deux se
        // lisent ensemble — voila pourquoi ca a echoue, voila ce qu'il reste a faire — et
        // la colonne d'actions est etroite : trois noms d'hebergeurs s'y empilaient en
        // colonne, un par ligne.
        tdEtat.append(dehors);
        // Le bouton qui vient d'echouer disparait : le garder proposerait de refaire
        // exactement ce qui n'a pas marche.
        bouton.remove();
      }
    }
  }
  ...
  /**
   * OU OUVRIR CE NUMERO : le service qui le sert, et le fichier exact.
   *
   * TROIS PROVENANCES, ET ELLES N'ONT PAS LE MEME CHEMIN. Un tracker passe par un
   * debrideur, qui rend l'inventaire de la release. Un lien direct — bookys, zone-ebook —
   * n'a AUCUN cache de debrideur : sa source sert ses octets elle-meme, et c'est le
   * sondage des hebergeurs qui dit ou aller. Telegram et Z-Library portent leur fichier
   * dans leur identite.
   *
   * Ne connaitre que le premier chemin bloquait purement et simplement les deux autres :
   * « en effet mais la tu m'empeches carrement de le prendre », sur un numero bookys.
   */
  async function ouOuvrirCeNumero(c, cle, entree, tdEtat) {
    if (c.hash) {
      // LE CACHE D'ABORD, toujours : aucune lecture ne declenche un telechargement.
      const service = await trouverService(cle, tdEtat, entree.titre);
      if (!service) throw new Error('pas en cache chez vos debrideurs');
      tdEtat.replaceChildren(elem('span', 'etat etat--inconnu', 'ouverture…'));
      ...
      const f = (structure.fichiers || []).find((x) => x.estBd);
      if (!f) throw new Error(structure.motifArchive || 'aucun fichier lisible ici');
      // Ici le nom porte son extension : l'inventaire du debrideur rend des chemins.
      return { service, chemin: f.chemin };
    }

    if (String(cle).startsWith('ddl:')) {
      tdEtat.replaceChildren(elem('span', 'etat etat--inconnu', 'sondage des hebergeurs…'));
      const structure = await api(
        `/api/release/${encodeURIComponent(cle)}/fichiers`
          + `?titre=${encodeURIComponent(c.titre || '')}`
          + `&famille=${encodeURIComponent(entree.famille || '')}`,
        { method: 'POST' },
      );
      // L'ECHEC EMPORTE LA STRUCTURE SONDEE. C'est elle qui porte les adresses publiques
      // des hebergeurs, et sans elle la ligne ne peut proposer aucune sortie : elle
      // affichait le motif et gardait « Lire ce numero », qui venait d'echouer.
      const echec = (m) => Object.assign(new Error(m), { structure });
      const choix = hebergementUtilisable(structure);
      if (!choix) throw echec('aucun de vos debrideurs ne peut servir ce numero');
      // La sonde a lu les premiers octets : elle sait si ce fichier s'ouvre en ligne.
      const archive = structure.archive || {};
      if (issueArchive(archive) !== 'lire') {
        throw echec(archive.motif || 'ce numero ne se lit pas en ligne');
      }
      // Le format vient de la SONDE, pas du nom : c'est elle qui a lu « %PDF ».
      return { service: choix.service, chemin: choix.nomFichier || c.titre, format: archive.format };
    }

    // Telegram et Z-Library : le service se lit sur le prefixe de l'identite, et le
    // fichier EST cette identite. Rien a sonder, rien a verifier.
    return {
      service: String(cle).startsWith('tg:') ? 'telegram' : 'zlibrary',
      chemin: c.titre,
    };
  }

  /**
   * Le bouton principal d'une ligne, et ce qu'il promet.
   *
   * Trois gestes, un seul endroit ou ils se decident : combler un tome precis, lire un
   * numero, ou poser le raccourci de l'oeuvre. Il etait ecrit DEUX FOIS dans la ligne —
   * une fois pour les releases sans debrideur, une fois pour les autres — et les deux
   * copies auraient diverge a la premiere retouche.
   */
  function actionDeChoix(c, cle, entree, p, rafraichir, tdEtat, cible) {
    const numero = !cible && entree.parutions;
    const b = elem('button', 'principal',
      cible ? `Prendre ce ${uniteDe(entree)} ici` : numero ? 'Lire ce numero' : 'Choisir');
    b.addEventListener('click', () => (cible
      ? poserAppoint(c, cle, entree, cible, p, rafraichir, tdEtat, b)
      : numero
        ? lireNumero(c, cle, entree, p, rafraichir, tdEtat, b)
        : poserRaccourci(c, cle, entree, p, rafraichir, tdEtat, b)));
    return b;
  }
  ...
  function ligneRelease(c, entree, p, rafraichir, lignes, cible) {
    ...
    const tr = elem('tr');
    // UN NUMERO SE RECONNAIT A SA DATE, pas au nom de sa release.
    // « Le.Journal.De.Spirou.N°4611.26.Août.2026.FR.[Pdf]-NOTAG » ne se compare pas d'un
    // coup d'oeil au numero suivant ; « 26 aout 2026 » si. Le nom reste dessous : c'est
    // lui qui dit le format et le groupe, et il sert a reconnaitre une release.
    const tdNom = elem('td', 'col-nom');
    if (c.parution) {
      const tete = elem('div', 'parution', c.parution);
      // DEJA LU SE DIT ICI, et pas dans la colonne d'etat : celle-la parle du CACHE, une
      // autre question. Les confondre ferait lire « lu » comme « disponible ».
      if (parutionsLues(entree).has(c.parution)) {
        tete.append(elem('span', 'deja-lu', 'lu'));
        tr.classList.add('ligne-lue');
      }
      tdNom.append(tete, elem('div', 'titres', c.titre));
    } else {
      tdNom.textContent = c.titre;
    }
    tdNom.title = c.titre;
    tr.append(tdNom);
    ...
    if (natureHorsDebrideur) {
      ...
      tdActions.append(actionDeChoix(c, cle, entree, p, rafraichir, tdEtat, cible));
  ...
  function sortieDeSecours(entree, tomeId, rafraichir) {
    ...
    if (servi && servi.appoint) {
      const b = elem('button', 'discret', `Chercher une autre release pour ce ${uniteDe(entree)}`);
  ...
  /**
   * Les sorties « chez l'hebergeur », quand aucun debrideur ne peut servir.
   *
   * MEME GESTE QUE SUR LA RECHERCHE, ou il a ete ajoute d'abord : le fichier existe et le
   * navigateur l'ouvre, mais l'etagere expliquait l'impasse sans en proposer la sortie.
   *
   * UN BOUTON PAR HEBERGEUR, NOMME. « Ouvrir chez l'hebergeur » seul ne dit pas ou l'on
   * va, et deux hebergeurs n'ont ni le meme temps d'attente ni les memes exigences.
   *
   * C'est l'adresse PUBLIQUE de la fiche, pas un lien debride : rien du compte de
   * l'utilisateur n'y circule.
   */
  function sortiesHebergeurs(structure) {
    const vus = new Set();
    const liens = (structure.hebergeurs || []).filter((h) => {
      if (!h.url || vus.has(h.hebergeur)) return false;
      vus.add(h.hebergeur);
      return true;
    });
    if (!liens.length) return null;
    const bloc = elem('div', 'puces');
    bloc.append(elem('span', 'tranche-legende', 'Ouvrir hors debrideur :'));
    for (const h of liens) {
      const b = elem('button', 'discret', h.hebergeur);
      b.type = 'button';
      b.title = 'Ouvre la page de cet hebergeur dans un nouvel onglet, hors debrideur';
      // `noopener` : la page ouverte ne peut pas manipuler celle-ci.
      // `noreferrer` : l'hebergeur n'apprend pas d'ou vient la visite.
      b.addEventListener('click', () => window.open(h.url, '_blank', 'noopener,noreferrer'));
      bloc.append(b);
    }
    return bloc;
  }
  ...
  /**
   * OUVRIR AVEC LA BONNE SURFACE.
   *
   * Un PDF n'est pas un CBZ : la visionneuse enchaine des planches d'images, la liseuse
   * PDF rend des pages dessinees. Le constat est deja ecrit dans `app.js` — « envoyer un
   * PDF a la visionneuse ouvrait une surcouche vide » — mais la bibliotheque, elle,
   * envoyait TOUS ses fichiers a la visionneuse. Reprendre un journal depuis l'etagere
   * ouvrait donc un ecran vide, alors que le meme fichier s'ouvrait depuis la recherche.
   *
   * L'ENCHAINEMENT DE TOMES NE SUIT PAS SUR UN PDF : la liseuse PDF ne le connait pas.
   * C'est sans consequence la ou il sert — un periodique n'a pas de tome suivant.
   */
  function ouvrirFichier(options) {
    // LE FORMAT SU PASSE AVANT LE NOM DEVINE. Un lien direct ne porte pas toujours
    // d'extension — « Le Journal de Spirou - 5 Août 2026 » n'en a aucune — mais la sonde
    // a LU les premiers octets et sait que c'est un PDF. Se fier au seul nom envoyait ce
    // fichier a la visionneuse, qui allait chercher un ZIP et repondait « ce fichier n
    // est pas lisible en ligne, telechargez-le » sur un PDF parfaitement lisible.
    const pdf = options.format === 'pdf' || estPdf(options.fichier || options.titre);
    const surface = pdf ? window.LiseusePdf : window.Visionneuse;
    surface.ouvrir(options);
  }
  ...
  async function ouvrirLienDirect(entree, tomeId, reprise, sortie, surFermeture) {
    // `entree` EST ICI LA RELEASE SERVANTE, pas forcement le favori : l'appelant y a mis
    // la cle de l'appoint quand c'est un appoint qui sert ce tome.
    ...
    try {
      structure = await api(
        `/api/release/${encodeURIComponent(cle)}/fichiers`
          + `?titre=${encodeURIComponent(entree.titreRelease || entree.titre || '')}`
          // LE RAYON DECIDE DE LA FORME DE L'IDENTITE : voir `identiteTome`.
          + `&famille=${encodeURIComponent(entree.famille || '')}`,
        { method: 'POST' },
      );
    } catch (e) {
    ...
    if (!choix) {
      ...
      const dehors = sortiesHebergeurs(structure);
      if (dehors) sortie.append(dehors);
    ...
    if (issue !== 'lire') {
      ...
      // MEME QUAND L'ARCHIVE N'EST PAS LISIBLE EN LIGNE : un RAR se recupere chez son
      // hebergeur, et c'est souvent le geste le plus court.
      const horsDebrideur = sortiesHebergeurs(structure);
      if (horsDebrideur) sortie.append(horsDebrideur);
    ...
    // L'IDENTITE VIENT DU SERVEUR, ET ON N'Y TOUCHE PAS. Cette ligne la refabriquait en
    // `f:<nom>` des qu'elle n'en avait pas deja la forme — juste sous un commentaire qui
    // croyait s'aligner sur la recherche, alors que la recherche a cesse de recalculer
    // cette identite le jour ou `debrid.ts` s'est mis a la rendre. Elle a donc diverge en
    // sens inverse : « Mad Movies 406 2026.pdf » ouvert depuis la fiche entrait sous
    // `t406`, puis « Reprendre » le rouvrait sous `f:Mad Movies 406 2026.pdf` — deux
    // lignes de progression pour une seule lecture, et un carre de plus dans la grille.
    const nom = choix.nomFichier || entree.titreRelease || entree.titre;
    // Le repli ne sert qu'a une ouverture sans tome demande : jamais a corriger le serveur.
    const identite = tomeId || `f:${nom}`;
    // LIBGEN SE LIT DEPUIS SA COPIE LOCALE : meme attente qu'a l'ouverture depuis la
    // recherche, et immediate quand le fichier est deja la. Voir `precharge.js`.
    if (String(cle).startsWith('ddl:libgen:')) {
      try {
        if (!(await window.Precharge.attendre({ identite: cle, zone: sortie, api }))) return;
      } catch (e) {
        sortie.replaceChildren(elem('div', 'message', e.message));
        return;
      }
    ...
    ouvrirFichier({
      famille: entree.famille || undefined,
      // La sonde a lu les premiers octets : elle sait le format meme quand le nom se tait.
      ...
      oeuvre: entree.oeuvre,
      tome: identite,
      // CE QUE CETTE RELEASE SERT. Un lien direct sert UN fichier ; sans cette liste le
      // favori garde `tomes` vide et la pastille du numero lu reste en POINTILLE, comme
      // si aucune release ne le servait. Meme correctif qu'a l'ouverture depuis la fiche.
      tomes: [identite],
      reprise: reprise || 0,
      titreOeuvre: entree.titre,
      couverture: entree.couverture || undefined,
  ...
  async function ouvrirTome(entree, tomeId, reprise, sortie, surFermeture) {
    ...
    if (estLienDirect(cle)) {
      // `releaseServante` ET NON `entree` : c'est la meme regle que trois lignes plus
      // haut, et elle avait ete oubliee sur ce chemin-la. `ouvrirLienDirect` sonde
      // `entree.cleRelease` — le raccourci de l'OEUVRE — alors qu'un numero servi par un
      // appoint n'en depend pas. On sondait donc la release du raccourci, une empreinte
      // de tracker sans le moindre hebergeur, pour repondre « aucun hebergeur de cette
      // release n est joignable » sur un numero qui venait de s'ouvrir.
      await ouvrirLienDirect(releaseServante, tomeId, reprise, sortie, surFermeture);
      return;
    }
    ...
    sortie.replaceChildren(elem('div', 'vide', `ouverture du ${uniteDe(entree)}…`));
    try {
      ...
      if (!f) {
        ...
        sortie.replaceChildren(elem('div', 'message',
          `${libelleTome(tomeId, entree)} n est pas dans cette release. Votre progression est conservee.`));
        ...
        if (dispo.length) {
          ...
          for (const x of dispo) {
            const b = elem('button', 'pastille-tome', etiquette(x.tomeId, entree));
            b.type = 'button';
      ...
      ouvrirFichier({
        ...
        surFermeture,
        tomeSuivant: suivant ? { tome: suivant, libelle: libelleTome(suivant, entree) } : null,
  ...
  function carte(entree, rafraichir) {
    ...
    const { barre, lus, total, unite } = barreAvancement(entree);
    droite.append(barre);
    ...
    const morceaux = [`${lus} / ${total} ${unite}${total > 1 ? 's' : ''} lu${lus > 1 ? 's' : ''}`];
    if (cible && !estLu(cible)) {
      morceaux.push(`en cours ${libelleTome(cible.tome, entree)}, ${libellePosition(cible)}`);
    }
    ...
    const depart = cible ? cible.tome
      : entree.cleRelease && estOuvrageAttendu(entree)
        ? `${prefixeOuvrage(entree.positions) || 'epub'}:${entree.cleRelease}`
        : 't1';
    const reprendre = elem('button', 'principal',
      cible ? `Reprendre ${libelleTome(cible.tome, entree)}` : 'Commencer');
    reprendre.addEventListener('click', () => surClic(depart, cible ? cible.planche : 0));
    actions.append(reprendre);

    // POUR UN PERIODIQUE, LA LISTE DES NUMEROS EST LE SECOND GESTE, et elle se place donc
    // la. Elle etait bien la, mais en dernier et sous un autre nom : la porte que tout le
    // monde prenait etait la GRILLE, ou chacune des 406 cases ouvrait cette meme liste.
    // « J'avais les 406 numeros ou je pouvais cliquer dessus et choisir des releases
    // d'appoint. » En cessant de dessiner 405 numeros que personne n'a jamais eus, j'ai
    // referme cette porte sans en ouvrir une autre.
    //
    // UN SEUL BOUTON, PAS DEUX. « Changer de release » appelle exactement la meme chose :
    // pour un periodique, changer de release EST choisir un numero. Deux libelles pour un
    // geste laissent croire a deux gestes.
    //
    // UN CLIC, PAS UN CHARGEMENT AUTOMATIQUE : chaque ouverture interroge toutes les
    // sources, et le faire pour chaque titre a l'affichage de la bibliotheque couterait
    // une recherche complete par carte.
    if (entree.parutions) {
      const numeros = elem('button', 'discret', 'Numeros disponibles');
      numeros.title = 'Demande aux sources les numeros de ce titre, et attache celui que '
        + 'vous choisissez.';
      numeros.addEventListener('click', () => changerRelease(entree, rafraichir));
      actions.append(numeros);
    }
    ...
    if (tomes.length) {
      ...
      details.append(elem('summary', null, entree.parutions
        ? `Numeros lus (${tomes.length})`
        : `Tous les tomes (${tomes.length})`));
      const grille = grilleTomes(entree, surClic,
        // UN NUMERO EST SA RELEASE : on ne rattache pas une release arbitraire a une date.
        // « Prendre ce numero ici » liait la release du 19 aout a l'identite du numero du
        // 26 — un fait faux, ecrit en base, qui faisait repondre « ce numero n est pas
        // dans cette release » sur un numero parfaitement servi ailleurs. Un numero
        // manquant renvoie donc a la LISTE des numeros, ou chacun ouvre le sien.
        (tomeId) => (entree.parutions
          ? changerRelease(entree, rafraichir)
          : chercherPourTome(entree, tomeId, rafraichir)));
    ...
    if (appoints.length > 0) {
      ...
      for (const a of appoints) {
        ...
        tr.append(elem('td', null, tomesServis(a).map((id) => libelleTome(id, entree)).join(', ') || '—'));
