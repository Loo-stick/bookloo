...
export function makeEndpointConfig<T extends Record<string, unknown>>(
  ...
  try {
    if (fs.existsSync(configPath)) {
      const observateur = fs.watch(configPath, (eventType) => {
        if (eventType === 'change') {
          console.log(`[Config] ${fileName} modifie, rechargement`);
          setTimeout(load, 100).unref();
        }
      });
      // L'OBSERVATEUR NE DOIT PAS RETENIR LE PROCESSUS. En production le serveur vit de
      // toute facon — son ecoute HTTP le maintient — et le rechargement a chaud continue
      // de fonctionner exactement pareil.
      //
      // Sans `unref`, tout programme qui importe un module a endpoint ne rend JAMAIS la
      // main. C'est ce qui obligeait la suite de tests a passer par `--test-force-exit`,
      // et ce drapeau coupait le processus pendant que des fichiers ecrivaient encore
      // leurs resultats : le nombre de tests VARIAIT d'une passe a l'autre — 1109, 1114,
      // 1118, 1124 sur un arbre identique — sans jamais le moindre echec. Un fichier
      // entier disparaissait en silence, et un test qui ne tourne pas ne peut pas rougir.
      observateur.unref();
    }
  } catch {
