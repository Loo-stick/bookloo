...
// PURGE UNIQUE DES ENTREES D'AVANT LE NETTOYAGE DES LIENS.
//
// Jusqu'au 2026-08-29, les reponses de tracker etaient mises en cache TELLES QUELLES, et
// leurs liens de telechargement portent la cle de l'utilisateur qui a declenche la requete.
// Releve dans une vraie base : « ?t=get&id=…&apikey=<cle> », ecrit sur disque et servi au
// compte suivant, le cache etant partage.
//
// Les entrees ecrites depuis sont anonymes, mais les anciennes vivent jusqu'a leur
// expiration. On les efface au demarrage : un TTL de trente minutes ne justifie pas de
// laisser une cle sur disque, et cette purge ne coute qu'une recherche a refaire.
try {
  const efface = db
    .prepare("DELETE FROM cache WHERE scope IN ('torznab', 'unit3d') AND value LIKE '%apikey=%'")
    .run().changes
    + db
      .prepare("DELETE FROM cache WHERE scope IN ('torznab', 'unit3d') AND value LIKE '%api_token=%'")
      .run().changes;
  if (efface > 0) console.log(`[Cache] ${efface} entree(s) porteuse(s) de cle effacee(s)`);
} catch {
  /* Une base plus ancienne peut ne pas avoir ces colonnes : la purge n'est pas critique. */
}
