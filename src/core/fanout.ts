...
export async function interrogerSource(
  ...
  try {
    ...
    // `userId` EST DANS LA MEME LISTE, et pour la meme raison : ce contexte est
    // RECONSTRUIT, champ par champ. Tout ce qu'on oublie ici est silencieusement perdu.
    const candidats = await source.search(
      q,
      { cles: ctx.cles, famille: ctx.famille, userId: ctx.userId, signal },
    );
