...
export interface DebridService {
  ...
  /**
   * Les fichiers d'une release EN CACHE, sans rien deposer sur le compte.
   *
   * Lister n'est pas lire : consulter ce qu'une release contient ne doit rien ajouter chez
   * le debrideur. TorBox l'expose vraiment — `checkcached` accepte `list_files=true` et ne
   * touche a rien. AllDebrid n'a aucune consultation en lecture seule, et rend donc `null` :
   * l'appelant retombe alors sur le depot, qui reste le seul chemin possible chez lui.
   *
   * `null` signale « je ne sais pas faire », jamais « la release est vide ». La distinction
   * est celle de tout le projet : ne pas savoir n'est pas savoir que non.
   */
  listerDepuisCache?(hash: string, signal?: AbortSignal): Promise<FichierDebride[] | null>;
