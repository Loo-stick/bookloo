...
/**
 * Le chemin sans sa requete.
 *
 * UNE TRACE NE DOIT JAMAIS POUVOIR PORTER UN SECRET. `requestdl` passait la cle en
 * parametre — c'est corrige — mais la regle ne doit pas dependre de ce qu'une URL contient
 * aujourd'hui : on journalise ce qui diagnostique, jamais ce qui identifie.
 *
 * Signale par une revue de code le 2026-08-29 : une simple coupure reseau ecrivait la cle
 * TorBox d'un utilisateur en clair dans les journaux du serveur.
 */
function sansRequete(chemin: string): string {
  const i = chemin.indexOf('?');
  return i === -1 ? chemin : chemin.slice(0, i);
}
...
async function tbGet<T>(chemin: string, apiKey: string, signal?: AbortSignal): Promise<T | null> {
  try {
    ...
    tracer('TorBox', `${sansRequete(chemin)}: ${(e as Error).message}`);
    return null;
  }
}
...
/**
 * Les fichiers d'une release en cache, lus dans une reponse de `checkcached`.
 *
 * Fonction pure : testee sans reseau, sur la forme MESUREE le 2026-08-28.
 *
 * Rend `null` — jamais une liste vide — quand la release n'est pas dans le cache ou que la
 * reponse ne porte pas de fichiers. « Ne pas savoir » n'est pas « savoir que non » : une
 * liste vide ferait croire a une release sans contenu, et l'appelant renoncerait au lieu de
 * deposer, qui reste le chemin de secours.
 */
export function parseFichiersEnCache(corps: unknown, hash: string): FichierDebride[] | null {
  const data = (corps as { data?: Record<string, unknown> } | null)?.data;
  if (!data || typeof data !== 'object') return null;

  const h = hash.toLowerCase();
  const cle = Object.keys(data).find((k) => k.toLowerCase() === h);
  if (!cle) return null;

  const fichiers = (data[cle] as { files?: unknown } | null)?.files;
  if (!Array.isArray(fichiers) || fichiers.length === 0) return null;

  return fichiers.map((f) => {
    const x = f as { id?: number; name?: string; short_name?: string; size?: number };
    return {
      id: x.id,
      // LE CHEMIN COMPLET, pas `short_name` : celui-ci perd l'arborescence, dont depend la
      // lecture d'un tome contenu dans une archive.
      nom: x.name || x.short_name || '',
      taille: x.size,
    };
  }) as FichierDebride[];
}
...
/**
 * Le chemin de `requestdl`, AVEC son `token`.
 *
 * CE PARAMETRE N'EST PAS REDONDANT, et l'avoir cru a casse la lecture depuis TorBox.
 * `requestdl` est le seul endpoint de cette API qui n'accepte PAS l'en-tete
 * `Authorization` : sa documentation officielle le dit en toutes lettres — « No
 * authorization header is required for this endpoint - instead, the API key is passed as
 * a query parameter called `token` ». Sans lui, l'appel echoue, `lienFichier` rend `null`,
 * et l'utilisateur lit « aucun lien de telechargement obtenu » sur une release pourtant
 * en cache. `lienZip` passe par le meme endpoint : l'archive rendait 502.
 *
 * IL AVAIT ETE RETIRE POUR PROTEGER LES JOURNAUX, le 2026-08-29, et cette inquietude
 * etait deja traitee par ailleurs : `sansRequete()` coupe la chaine de requete AVANT
 * toute trace, quoi qu'elle contienne. C'etait la garantie robuste — elle ne depend
 * d'aucune liste de noms de parametres. Retirer le parametre EN PLUS n'ajoutait rien, et
 * cassait la lecture.
 *
 * `fileId` a `null` demande l'archive entiere.
 */
export function urlRequestdl(
  apiKey: string, torrentId: number | string, fileId: number | string | null,
): string {
  const params = new URLSearchParams({
    token: apiKey,
    torrent_id: String(torrentId),
    zip_link: fileId === null ? 'true' : 'false',
  });
  if (fileId !== null) params.set('file_id', String(fileId));
  return `/torrents/requestdl?${params.toString()}`;
}
  ...
  return {
    ...
    async deposer(hash, torrentUrl, signal) {
      ...
      if (torrentUrl) {
        ...
        // ET PAS DAVANTAGE QUAND LE DEPOT A ECHOUE. Le paragraphe ci-dessus ne valait que
        // pour le succes : un tracker en panne, un timeout ou une page d'erreur a la
        // place du bencode faisaient tomber le code sur le magnet nu — le piege exact
        // qu'il pretend eviter. Signale par une revue le 2026-08-29.
    ...
    /**
     * Les fichiers d'une release EN CACHE, sans rien deposer.
     *
     * `checkcached` est une consultation — le commentaire en tete de ce fichier le dit
     * depuis l'origine : « accepte un lot de hashs, ne depose rien ». Il accepte aussi
     * `list_files=true`, et c'est ce qui permet d'afficher la grille des tomes d'une
     * release sans l'ajouter au compte de l'utilisateur.
     */
    async listerDepuisCache(hash, signal) {
      const h = hash.toLowerCase();
      if (!estHash(h)) return null;
      const corps = await tbPost<unknown>(
        '/torrents/checkcached?format=object&list_files=true',
        apiKey,
        { hashes: [h] },
        false,
        signal,
      );
      // `null` remonte tel quel : une panne de reseau ne se confond pas avec une release
      // absente du cache, et l'appelant retombe sur le depot dans les deux cas.
      return corps === null ? null : parseFichiersEnCache(corps, h);
    },
    ...
    async lienFichier(idOuHash, idFichier, signal) {
      ...
      const dl = await tbGet<{ url?: string } | string>(
        urlRequestdl(apiKey, t.id, idFichier),
        apiKey,
        signal,
      );
