  ...
  return {
    ...
    async lienFichier(idOuHash, idFichier, signal) {
      ...
      const dl = await tbGet<{ url?: string } | string>(
        // SANS `token=` : l'en-tete Authorization porte deja la cle. Le paramètre etait
        // redondant, et il finissait dans les traces d'erreur.
        '/torrents/requestdl?' +
    ...
    async lienZip(idOuHash, signal) {
      ...
      if (!t) return null;
      const dl = await tbGet<{ url?: string } | string>(
        `/torrents/requestdl?torrent_id=${t.id}&zip_link=true`,
        apiKey,
        signal,
      );
      if (typeof dl === 'string') return dl;
