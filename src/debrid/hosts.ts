...
export function parseHotesAllDebrid(corps: unknown): Set<string> {
  const hosts = (corps as {
    data?: { hosts?: Record<string, { status?: boolean; domains?: unknown }> };
  })?.data?.hosts;
  ...
  for (const [nom, info] of Object.entries(hosts)) {
    ...
    // LES DOMAINES COMPTENT AUTANT QUE LE NOM, comme chez TorBox deux fonctions plus bas.
    // Seul le nom etait lu ici : mesure du 2026-09-11, turbobit declare dix-sept domaines
    // — `turbobit.cloud`, `htfl.net`, `trbbt.net`… — et un lien publie sur l'un d'eux
    // etait annonce « non pris en charge » par AllDebrid, « pris en charge » par TorBox.
    if (Array.isArray(info?.domains)) {
      for (const d of info.domains) if (typeof d === 'string') out.add(normaliserHote(d));
    }
  }
  return out;
}
...
export async function hotesSupportes(service: NomService, cle: string): Promise<Set<string>> {
  ...
  const liste = await cached<string[] | null>(
    // `v2` : le cache garde la liste DEJA ANALYSEE. Sans changer de cle, celle d'avant la
    // lecture des domaines AllDebrid aurait survecu douze heures au correctif.
    `hotes:v2:${service}`,
    TTL_MS,
    async () => {
      try {
