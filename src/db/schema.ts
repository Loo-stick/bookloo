...
import { identiteRattrapee, positionAGarder } from '../bibliotheque/reprise-numeros';
...
export function ouvrirMangaloo(fichier?: string): Database {
  ...
  db.exec(`
    ...
    -- CHAQUE REQUETE AUTHENTIFIEE CHERCHE PAR « token_hash ». Sans index c'est un balayage
    -- complet a chaque appel, et la table des sessions grossit avec l'usage.
  ...
  rattraperNumerosDePresse(db);

  return db;
}

/**
 * Ramene les numeros de presse entres sous une identite de TOME.
 *
 * La regle vit dans `bibliotheque/reprise-numeros.ts`, ou elle est testee. Ici il n'y a
 * que la lecture, l'ecriture et le compte rendu.
 *
 * IDEMPOTENT : une fois la ligne reecrite, `identiteRattrapee` ne la reconnait plus, et
 * les demarrages suivants ne trouvent rien. C'est la meme garantie que `ajouterColonne`.
 */
function rattraperNumerosDePresse(db: Database): void {
  const favoris = db
    .prepare('SELECT user_id, oeuvre, famille, titre_release, tomes FROM favoris')
    .all() as {
      user_id: number; oeuvre: string; famille: string | null;
      titre_release: string | null; tomes: string | null;
    }[];
  if (!favoris.length) return;

  const lire = db.prepare(
    'SELECT tome, planche, planches, pourcent FROM progression'
  ...
  const effacer = db.prepare(
    'DELETE FROM progression WHERE user_id = ? AND oeuvre = ? AND tome = ?',
  );
  // CE QUE LA RELEASE SERT, quand elle ne le disait pas. La liseuse PDF recevait cette
  // liste et la jetait : le favori restait a `tomes` vide, donc la pastille du numero lu
  // s'affichait en POINTILLE — « aucune release ne sert ce numero » — et renvoyait
  // chercher une release qu'on avait deja. Reecrire la position sans reparer ca laisserait
  // le carre exactement aussi barre qu'avant.
  //
  // On ne l'ecrit que sur un favori qu'on vient de rattraper et dont la liste est VIDE :
  // une liste existante a ete posee par quelqu'un qui en savait plus que nous.
  const declarer = db.prepare(
    'UPDATE favoris SET tomes = ? WHERE user_id = ? AND oeuvre = ?'
    + " AND (tomes IS NULL OR tomes = '' OR tomes = '[]')",
  );

  let reecrites = 0;
  for (const f of favoris) {
    const positions = lire.all(f.user_id, f.oeuvre) as {
      tome: string; planche: number; planches: number; pourcent: number | null;
    }[];
    for (const p of positions) {
      const voulue = identiteRattrapee(f.famille, f.titre_release, p.tome);
      if (!voulue) continue;
      const garde = positionAGarder(p, positions.find((q) => q.tome === voulue));
      db.transaction(() => {
        ecrire.run(
          f.user_id, f.oeuvre, voulue,
          garde.planche, (garde as typeof p).planches, (garde as typeof p).pourcent ?? null,
          Date.now(),
        );
        effacer.run(f.user_id, f.oeuvre, p.tome);
        declarer.run(JSON.stringify([voulue]), f.user_id, f.oeuvre);
      })();
      reecrites += 1;
    }
  }
  if (reecrites) console.log(`[Base] ${reecrites} numero(s) de presse ramene(s) a leur nom de fichier`);
}
...
export const NOMS_CLES = [
  ...
  'zlib_email', 'zlib_mdp',
  // Zone-ebook : le PSEUDO, jamais l'adresse. Mesure du 2026-09-02 sur le vrai site — la
  // connexion par adresse ne pose que `PHPSESSID`, celle par pseudo pose `dle_user_id`.
  'zone_login', 'zone_mdp',
  // Comics Tracker : l'ADRESSE et le mot de passe. Son authentification est Firebase
  // (mesure du 2026-09-16, cle web publique lue dans son JS) : la connexion se fait par
  // e-mail, et rend un jeton d'une heure.
  'ct_email', 'ct_mdp',
] as const;
