...
export async function sessionZlibraryDe(
  ...
  const connue = retenue<Session>('zlibrary', userId);
  ...
  retenir('zlibrary', userId, s);
  return s;
}

/**
 * Oublie la session d'un utilisateur.
 *
 * Appelee quand le serveur la refuse, ET par `routes/compte.ts` quand les identifiants
 * changent — sans quoi une correction de mot de passe restait sans effet visible.
 */
export function oublierSession(userId: number): void {
  oublier('zlibrary', userId);
}

export function routesZlibrary(db: Database): Router {
  const r = Router();

  r.get('/api/zlibrary/telechargement/:identite', exigerConnexion, exigerMemeOrigine, async (req, res) => {
