...
async function ouvrir(
  ...
  if (!estIdentiteTelegram(identite) && !identite.startsWith('zl:')) {
    ...
    // LE VERDICT PORTE DEJA SON STATUT : le jeter transformait un refus de contrat en
    // panne de service distant.
    if (estVerdict(d)) throw new ErreurZlibrary(d.message ?? 'lecture refusee', d.code ?? 400);
    if (d.nom === 'telegram') throw new ErreurZlibrary('identite de livre invalide');
    // LE DISPATCH COMMUN, ET NON LE SEUL CHEMIN DEBRIDEUR. Il sait deja ouvrir un fichier
    // sans cle quand la source le permet — c'est le cas de fourtoutici depuis le
    // 2026-09-07. Pour tout le reste, il appelle exactement ce qui etait appele ici.
...
function repondreErreur(res: import('express').Response, e: unknown): void {
  if (e instanceof ErreurZlibrary) { res.status(e.statut).json({ erreur: e.message }); return; }
...
export function routesLectureEpub(db: Database): Router {
  ...
  r.get('/api/epub/:identite/sommaire', exigerConnexion, exigerMemeOrigine, async (req, res) => {
    try {
  ...
  r.get('/api/epub/:identite/chapitre/:n', exigerConnexion, exigerMemeOrigine, async (req, res) => {
  ...
  r.get('/api/epub/:identite/ressource', exigerConnexion, exigerMemeOrigine, async (req, res) => {
