 ...
 *
 * La regle ne s'ecrit qu'ici. Pour une oeuvre a tomes : plage de tomes la plus large, puis
 * seeders.
 *
 * POUR UN PERIODIQUE : LE PLUS RECENT EN TETE. « Quand on est sur les resultats, c'est
 * possible de classer les numeros du Parisien du plus recent au plus vieux ? » — ils
 * sortaient par seeders, donc dans un ordre sans rapport avec leur parution. La date
 * (`paruLe`) n'est posee que pour une famille qui ne compte pas en tomes — voir
 * `diffuser` — si bien que cette regle est propre a la presse par construction.
 *
 * UN NUMERO DATE PASSE AVANT UN NUMERO SANS DATE, qui garde la regle des seeders derriere :
 * rester visible sans pretendre a une place. Un mensuel (« septembre 2026 ») compte pour le
 * premier du mois. Et deux numeros du meme jour se departagent par leurs seeders.
 *
 * Les grandeurs restent tres en deca de la precision d'un nombre : ≈ 20 700 jours depuis
 * 1970, fois un million, plus un decalage de 10¹³.
 */
export function rangDe(c: Candidate): number {
  const seeders = Math.min(c.seeders ?? 0, 999_999);
  const jours = joursDepuis1970(c.paruLe);
  if (jours !== null) return -(10_000_000_000_000 + jours * 1_000_000 + seeders);
  const etendue = c.tomes ? (c.tomes.fin ?? c.tomes.debut) - c.tomes.debut + 1 : 0;
  return -(etendue * 1_000_000 + seeders);
}

/** `2026-09-08` ou `2026-09` -> jours depuis le 1er janvier 1970, ou `null`. */
function joursDepuis1970(date: string | undefined): number | null {
