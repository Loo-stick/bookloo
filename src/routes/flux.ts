...
import type { Candidate, Query, ResultatSource } from '../sources/types';
import { compteAuRatio } from '../sources/ratio';
import { familleDemandee } from './famille';
import { sourceEcartee, formatEcarte, type Famille } from '../familles/table';
...
async function diffuser(
  ...
  const minuteur = setTimeout(() => controleur.abort(), settings.fanoutBudgetMs);
  ...
  try {
    ...
    const travaux = sources.map((s) =>
      interrogerSource(s, q, { cles: clesDeRecherche(cles), famille }, controleur.signal),
    );
    ...
    for await (const r of auFilDeLEau<ResultatSource>(travaux)) {
      ...
      const gardes = r.candidats.filter((c) => !formatEcarte(famille, c.format));
...
export function routesFlux(db: Database): Router {
  ...
  r.get('/api/recherche-libre/flux', exigerConnexion, limiteurRecherche, async (req, res) => {
    ...
    await diffuser(db, req, res, null, { titres: [q] }, famille);
  });
