...
import { sourcesInvalidees, oublier as oublierSession } from '../auth/sessions-sources';
...
import { identifiantsZoneEbook } from '../presse/zone-ebook-session';
...
export function routesCompte(db: Database): Router {
  ...
  // MEME PARTAGE QUE POUR Z-LIBRARY : le pseudo revient, le mot de passe jamais. Sans le
  // pseudo, on ne peut pas voir quel compte est enregistre, et une faute de frappe reste
  // invisible pour toujours.
  r.get('/api/compte/zone-ebook', exigerConnexion, (req, res) => {
    const enClair = clesEnClair(db, req.session!.userId, req.session!.dek);
    res.json({
      login: (enClair.zone_login ?? '').trim() || null,
      complet: identifiantsZoneEbook(enClair) !== null,
    });
  });

  // MEME PARTAGE ENCORE : l'adresse revient, le mot de passe jamais. Comics Tracker se
  // connecte par e-mail — son authentification est Firebase.
  r.get('/api/compte/comics-tracker', exigerConnexion, (req, res) => {
    const enClair = clesEnClair(db, req.session!.userId, req.session!.dek);
    res.json({
      email: (enClair.ct_email ?? '').trim() || null,
      complet: identifiantsComicsTracker(enClair) !== null,
    });
  });
  ...
  r.put('/api/compte/cles', exigerConnexion, exigerCsrf, (req, res) => {
    ...
    // LES NOUVEAUX IDENTIFIANTS DOIVENT SERVIR TOUT DE SUITE. Sans cet oubli, la session
    // Z-Library retenue en memoire continuait a servir jusqu'a ce que Z-Library la
    // refuse : l'utilisateur corrigeait un mot de passe faux et relisait exactement la
    // meme erreur, sans rien qui lui dise que sa correction avait ete enregistree.
    // Signale par une revue de code le 2026-08-29.
    // CHAQUE source dont les cles viennent de changer oublie sa session. Sans cela, on
    // corrige un mot de passe faux et l'ancienne session continue de rendre la meme
    // erreur, sans aucun moyen de comprendre que la correction a bien ete prise.
    for (const s of sourcesInvalidees(modifiees)) oublierSession(s, req.session!.userId);
