...
import { changementInvalideSession, oublier as oublierSessionZlib } from '../sources/zlibrary/memoire-sessions';
...
export function routesCompte(db: Database): Router {
  ...
  r.put('/api/compte/cles', exigerConnexion, exigerCsrf, (req, res) => {
    ...
    if (changementInvalideSession(modifiees)) oublierSessionZlib(req.session!.userId);
