...
import { formatRar, sommaireRar } from '../lecture/rar';
...
async function obtenirSommaire(userId: number, d: Demande): Promise<CbzOuvert> {
  ...
  if (imbrique) {
    ...
    const sommaireInterne = await sommaireArchive(lireDedans, entree.tailleReelle);
    const ouvertInterne = {
  ...
  const sommaire = await sommaireArchive(distant.lire, distant.taille);
 ...
 */
export async function sommaireArchive(lire: LecteurPlage, taille: number) {
  try {
    ...
    return sommaireRar(lire, taille);
  }
}
...
export async function ouvrirDistantDebrid(
  ...
  const cible = fichiers.find((f) => base(f.nom) === base(demande));
