...
import { estRefuse } from '../debrid/refus-debrideur';
import { hotesSupportes } from '../debrid/hosts';
import { declaresDabord, motifSansLienDirect } from '../debrid/tentable';
...
import { prechargesLibgen } from '../sources/ddl/libgen';
import { ouvrirLocal } from '../lecture/precharges';
import { estIdentiteCt, idCt } from '../sources/comics-tracker/recherche';
import { identifiantsComicsTracker } from '../sources/comics-tracker/session';
...
export type Demande =
  ...
  // Servie SANS DEBRIDEUR, comme Telegram : le site rend le fichier lui-meme et honore
  // les plages. La `cle` vide n'est pas un oubli, c'est le fait.
  | { nom: 'fourtoutici'; cle: ''; hash: string; fichier: string }
  // LibGen aussi : son CDN sert le fichier et honore les plages. L'adresse, elle, se RESOUT
  // a l'ouverture — voir `sources/ddl/libgen.ts`.
  | { nom: 'libgen'; cle: ''; hash: string; fichier: string }
  // Comics Tracker : ses fichiers sont chez Google Drive, qui honore les plages — 1,2 Mo/s
  // mesure du 2026-09-16. Mais l'adresse se demande AVEC LE COMPTE du membre : ses
  // identifiants voyagent donc dans la demande, comme la cle d'un debrideur.
  | { nom: 'comics-tracker'; cle: ''; hash: string; fichier: string; ids: { email: string; mdp: string } };
...
export function resoudreDemande(
  ...
  // FOURTOUTICI AVANT LA VALIDATION DE SERVICE, pour la meme raison que Telegram : le
  // garde-fou plus bas exige une cle de debrideur, et cette source n'en demande aucune.
  // Mesure du 2026-09-07 : `download.php` rend un 206 avec un `Content-Range` correct, le
  // fichier se lit donc directement.
  if (estIdentiteFourtoutici(brut)) {
    const fichierFtc = String(query.fichier ?? '').trim();
    if (fichierRequis && !fichierFtc) {
      return { ok: false, code: 400, message: 'aucun fichier demande' };
    }
    return { nom: 'fourtoutici', cle: '', hash: brut, fichier: fichierFtc };
  }

  // LIBGEN AVANT LA VALIDATION DE SERVICE, pour la meme raison : aucune cle de debrideur
  // n'est necessaire. Mesure du 2026-09-15 : son CDN rend un 206 avec un `Content-Range`
  // correct, et annonce la taille au HEAD.
  if (estIdentiteLibgen(brut)) {
    const fichierLg = String(query.fichier ?? '').trim();
    if (fichierRequis && !fichierLg) {
      return { ok: false, code: 400, message: 'aucun fichier demande' };
    }
    return { nom: 'libgen', cle: '', hash: brut, fichier: fichierLg };
  }

  // COMICS TRACKER AVANT LA VALIDATION DE SERVICE, pour la meme raison — aucun debrideur —
  // mais avec une difference : IL FAUT LE COMPTE DU MEMBRE. On le lit ici, la ou la cle de
  // dechiffrement est disponible, et on le porte dans la demande.
  if (estIdentiteCt(brut)) {
    const fichierCt = String(query.fichier ?? '').trim();
    if (fichierRequis && !fichierCt) {
      return { ok: false, code: 400, message: 'aucun fichier demande' };
    }
    const ids = identifiantsComicsTracker(clesEnClair(db, userId, dek));
    if (!ids) {
      return {
        ok: false, code: 400,
        message: 'aucun compte Comics Tracker enregistre — ajoutez votre adresse et votre mot de passe dans Compte',
      };
    }
    return { nom: 'comics-tracker', cle: '', hash: brut, fichier: fichierCt, ids };
  }
...
async function obtenirSommaire(
  ...
  // SANS DEBRIDEUR, comme Telegram : le site sert le fichier et honore les plages. Le
  // lecteur par plages ne change pas d'un iota — c'est tout l'interet d'avoir separe
  // « obtenir le fichier distant » de « en lire le sommaire ».
  if (d.nom === 'fourtoutici' || d.nom === 'libgen' || d.nom === 'comics-tracker') {
    return ouvrirSansDebrideur(userId, d, avancee);
  }
...
/**
 * Ouvre une release servie DIRECTEMENT par son site, sans debrideur.
 *
 * Meme forme que `ouvrirTelegram`, et pour la meme raison : `ouvrirDistant` lit une
 * adresse comme une autre, et ni `plages.ts` ni `zip.ts` ne savent d'ou elle vient.
 */
async function ouvrirSansDebrideur(
  userId: number,
  d: Extract<Demande, { nom: 'fourtoutici' | 'libgen' | 'comics-tracker' }>,
  avancee?: Avancee,
): Promise<CbzOuvert> {
  const distant = await distantDeLaDemande(userId, d);
  const sommaire = await sommaireArchive(distant.lire, distant.taille, avancee);
  const ouvert = { sommaire, distant };
  poserSommaire(userId, d.hash, d.fichier, ouvert);
  return ouvert;
...
/**
 * Le fichier distant d'une release en LIEN DIRECT : resolution DDL au lieu de depot de
 * torrent. Separe de `ouvrirDdl` pour que les surfaces sans sommaire — la liseuse PDF —
 * puissent l'obtenir sans reecrire le dispatch.
 */
async function distantDdl(d: Extract<Demande, { nom: NomService }>) {
  const m = d.hash.match(/^ddl:([a-z0-9-]+):([A-Za-z0-9_-]+)$/);
  if (!m) throw new Error('identite de lien direct invalide');
  // UNE TABLE, ET PLUS UN NOM EN DUR. « wawacity » etait code ici : le kiosque de presse
  // produit des identites `ddl:zone-ebook:<id>`, qui tombaient toutes en « identite
  // invalide » alors que le reste du chemin — deverrouillage, hebergeurs, liens morts —
  // les traite exactement pareil. L'erreur NOMME le site inconnu, plutot que de laisser
  // croire que l'identite est malformee.
  const ouvreur = ouvreurDdl(m[1]);
  if (!ouvreur) {
    throw new Error(`site de lien direct inconnu : ${m[1]} (connus : ${SITES_DDL.join(', ')})`);
  }

  const liens = await ouvreur(m[2]);
  if (!liens?.length) throw new Error('la fiche n a pas pu etre ouverte');

  // Seul un service qui sait debloquer un lien direct ICI peut servir, et sa liste
  // d'hebergeurs ordonne les essais sans les interdire. Voir `debrid/tentable.ts`.
  const sansChemin = motifSansLienDirect(d.nom);
  if (sansChemin) throw new Error(sansChemin);
  const supportes = await hotesSupportes(d.nom, d.cle);
  const utilisables = declaresDabord(liens.filter((l) => !estRefuse(d.nom, l.url)), supportes);
  if (utilisables.length === 0) {
    throw new Error(`${d.nom} a deja refuse tous les liens de cette fiche`);
  }
  ...
  const resoudre = async (): Promise<string | null> => {
    for (const l of utilisables) {
      if (estRefuse(d.nom, l.url)) continue;
...
async function ouvrirDdl(
  ...
  const distant = await distantDdl(d);
  const sommaire = await sommaireArchive(distant.lire, distant.taille, avancee);
  const ouvert = { sommaire, distant };
  poserSommaire(userId, d.hash, d.fichier, ouvert);
  return ouvert;
...
/**
 * Le fichier distant d'une demande, QUELLE QUE SOIT SA PROVENANCE, sans lire de sommaire.
 *
 * POURQUOI ELLE EXISTE. Le dispatch — Telegram, lien direct, debrideur — vivait dans
 * `obtenirSommaire`, qui construit AUSSI un sommaire de planches. Une surface qui n'en a
 * pas besoin, comme la liseuse PDF, aurait donc du reecrire ce dispatch : c'est
 * exactement la duplication qui a fait qu'une release bookys parfaitement trouvee refusait
 * de s'ouvrir, le 2026-09-01, parce que la liste des sites vivait a trois endroits.
 */
export async function distantDeLaDemande(userId: number, d: Demande) {
  if (d.nom === 'telegram') {
    return ouvrirDistant({
      resoudre: async () => urlFichier(d.hash),
      enTetes: enTetesFacade(),
    });
  }
  if (d.nom === 'fourtoutici') {
    // Aucun en-tete, aucune cle : l'adresse est publique et sert le fichier tel quel.
    return ouvrirDistant({ resoudre: async () => urlFichierFourtoutici(idFourtoutici(d.hash)) });
  }
  if (d.nom === 'libgen') {
    // LA COPIE LOCALE, TOUJOURS. Le CDN de LibGen rend 14 a 400 Ko/s : une planche lue a
    // distance prenait 19 s. L'interface precharge le fichier AVANT d'ouvrir une liseuse —
    // voir `lecture/precharges.ts` et `web/precharge.js`. Arriver ici sans copie veut dire
    // qu'on a contourne ce passage, ou que le fichier a ete evince depuis.
    const chemin = prechargesLibgen().cheminPret(md5Libgen(d.hash));
    if (!chemin) {
      throw new Error("ce fichier LibGen n'est pas encore precharge — rouvrez-le depuis la recherche ou la bibliotheque");
    }
    return ouvrirLocal(chemin);
  }
  if (d.nom === 'comics-tracker') {
    // UNE ADRESSE FRAICHE A CHAQUE APPEL : le `uuid` de Drive est propre a la demande, et le
    // jeton du membre ne vaut qu'une heure. Voir `sources/comics-tracker/ouverture.ts`.
    return ouvrirDistant({
      resoudre: resolveurCt(userId, { ct_email: d.ids.email, ct_mdp: d.ids.mdp }, idCt(d.hash)),
    });
  }
  if (d.hash.startsWith('ddl:')) return distantDdl(d);
  return ouvrirDistantDebrid(userId, d);
}
...
export function routesLecture(db: Database): Router {
  ...
  r.get('/api/lecture/:hash/sommaire', exigerConnexion, exigerMemeOrigine, async (req, res) => {
   ...
   */
  r.get('/api/lecture/:hash/sommaire/flux', exigerConnexion, exigerMemeOrigine, async (req, res) => {
    const d = resoudreDemande(db, req.session!.userId, req.session!.dek, req.params, req.query);
    if (estVerdict(d)) {
      res.status(d.code ?? 400).json({ erreur: d.message });
      return;
  ...
  r.get('/api/lecture/:hash/page/:n', exigerConnexion, exigerMemeOrigine, async (req, res) => {
    const d = resoudreDemande(db, req.session!.userId, req.session!.dek, req.params, req.query);
    if (estVerdict(d)) {
      res.status(d.code ?? 400).json({ erreur: d.message });
      return;
