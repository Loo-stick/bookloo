...
export function routesAuth(db: Database): Router {
  ...
  r.post('/api/login', limiteurConnexion, async (req, res) => {
    ...
    // LE MOT DE PASSE D'ABORD, LE VERROU ENSUITE. Ce fichier prend soin de rendre le meme
    // refus pour « login inconnu » et « mauvais mot de passe » — mais un 429 « compte
    // verrouille » ne peut apparaitre que si le login EXISTE : teste avant le mot de passe,
    // il confirmait a lui seul l'existence du compte. Signale par une revue de code le
    // 2026-08-29.
    //
    // Le cout : on hache desormais aussi sur un compte verrouille. `limiteurConnexion`
    // borne la cadence par IP, et le prix d'un hachage vaut mieux qu'un oracle gratuit.
    const bon = await verifierMotDePasse(mdp, u.hash);
    if (!bon) {
      // UN ECHEC SUR UN COMPTE DEJA VERROUILLE NE PROLONGE PAS LA PEINE : sinon un tiers
      // maintiendrait le compte ferme indefiniment en echouant en boucle, exactement le
      // deni de service que le verrouillage temporaire cherche a eviter.
      if (!estVerrouille(u)) enregistrerEchec(db, login);
      await attendreUnPeu();
      res.status(401).json(REFUS);
      return;
    }

    // Le mot de passe est bon : on peut alors expliquer le verrou sans rien reveler a qui
