...
import { worldivxSource } from '../sources/torrent/worldivx';
import { wawacitySource } from '../sources/ddl/wawacity';
import { bookysSource } from '../sources/ddl/bookys';
import { fourtouticiSource } from '../sources/ddl/fourtoutici';
import { milleEbooksSource } from '../sources/ddl/1001ebooks';
import { downmagazSource } from '../sources/ddl/downmagaz';
import { libgenSource } from '../sources/ddl/libgen';
import { comicsTrackerSource } from '../sources/comics-tracker/source';
import { zoneEbookSource } from '../sources/ddl/zone-ebook';
...
export function toutesLesSources() {
  ...
  return [
    ...torznabSources(), ...unit3dSources(), nyaaSource(), knabenSource(),
    // Site public francophone : presse et livres en torrents a trackers publics. Voir
    // `sources/torrent/worldivx.ts`.
    worldivxSource(),
    wawacitySource(),
    bookysSource(), fourtouticiSource(),
    // Elle ne sert que Livres et Presse, et le dit elle-meme : les autres rayons
    // recoivent un tableau vide sans qu'une requete parte.
    milleEbooksSource(),
    // Presse et BD seulement ; lisible avec TorBox. Voir `downmagaz.ts`.
    downmagazSource(),
    // Livres et presse EN FRANCAIS, lisibles sans debrideur. Ses comics et ses mangas ont
    // ete ecartes : leurs fichiers ne s'ouvrent plus. Voir `sources/ddl/libgen-liste.ts`.
    libgenSource(),
    // Comics americains en editions FRANCAISES, lus depuis Google Drive avec le compte du
    // membre. Voir `sources/comics-tracker/source.ts`.
    comicsTrackerSource(),
    // Zone-ebook ne CHERCHE que pour ses membres : `cleRequise` le fait ecarter proprement
    // tant que le compte n'est pas renseigne, plutot que de rendre une liste vide qui se
...
/**
 * Ce qu'on a le droit de MEMORISER d'une verification de cache.
 *
 * UNE PANNE N'EST PAS UN VERDICT. Quand TorBox ne repond pas, chaque hash demande recoit
 * « inconnu » par defaut — c'est juste pour l'affichage, ou « on ne sait pas » est une
 * reponse honnete. Mais ces « inconnu » etaient ensuite ECRITS par-dessus les verdicts
 * deja connus, sans condition : une panne de quelques minutes faisait perdre en masse la
 * pastille « en cache » de contenus parfaitement lisibles la veille, jusqu'a la prochaine
 * verification reussie. Signale par une revue de code le 2026-08-29.
 *
 * DEUX REGLES, ET LA SECONDE COMPTE AUTANT QUE LA PREMIERE :
 *   - le service n'a pas repondu : on ne memorise RIEN ;
 *   - il a repondu, mais ce hash-la reste « inconnu » : on ne le memorise pas non plus.
 *     Ecrire « inconnu » ne dit rien qu'oublier ne dise deja, et cela EFFACE.
 */
export function verdictsAMemoriser(
  etats: Record<string, EtatCache>,
  service: EtatsDebrid['services']['torbox'],
): [string, EtatCache][] {
  if (service !== 'repondu') return [];
  return Object.entries(etats).filter(([, e]) => e !== 'inconnu');
}
...
/** Au-dela, une seule requete se transforme en rafale vers le debrideur. */
const MAX_HASHES = 200;

/**
 * Les empreintes demandees : dedoublonnees, validees, et PLAFONNEES.
 *
 * Sans plafond, l'amplification est offerte a n'importe quel compte — une requete, des
 * centaines d'appels sortants. Deux cents couvrent largement une page de resultats.
 * Signale par une revue de code le 2026-08-29.
 */
export function hashesDemandes(brut: unknown): string[] {
  ...
  return propres.slice(0, MAX_HASHES);
}
...
export function routesReleases(db: Database): Router {
  ...
  r.post('/api/etats-cache', exigerConnexion, exigerCsrf, async (req, res) => {
    const hashes = hashesDemandes((req.body as { hashes?: unknown })?.hashes);
    if (hashes.length === 0) {
    ...
    for (const [h, etat] of verdictsAMemoriser(etats.torbox, etats.services.torbox)) {
