...
import { oublierTout as oublierSessionsSources } from '../auth/sessions-sources';
...
export function routesAdmin(db: Database): Router {
  ...
  r.delete('/api/admin/utilisateurs/:id', exigerCsrf, (req, res) => {
    ...
    // ET LA SESSION Z-LIBRARY AVEC. Elle vit en memoire, pas en base : le CASCADE ne
    // l'emporte pas, et elle survivrait a la suppression du compte qu'elle authentifie.
    // TOUTES les sources, pas seulement Z-Library : un compte supprime ne doit laisser
    // aucune session ouverte, y compris celle d'une source ajoutee depuis.
    oublierSessionsSources(id);
