...
export function routesArchive(db: Database): Router {
  ...
  r.get('/api/release/:identite/archive', exigerConnexion, async (req, res) => {
    ...
    try {
      ...
      // UNE RELEASE EN LIEN DIRECT N'EST PAS UN TORRENT. `trouver` et `deposer` attendent une
      // EMPREINTE : une identite « ddl:… » y entrait telle quelle et faisait rendre au
      // debrideur un 502 incomprehensible. `obtenirSommaire` dispatche deja sur le prefixe ;
      // cette route ne le faisait pas. Signale par une revue de code le 2026-08-29.
      // Le test porte sur le NOM autant que sur la forme : `fourtoutici` est un lien
      // direct lui aussi, et c'est ce qui permet au typage de le savoir.
      if (d.nom === 'fourtoutici' || d.nom === 'libgen' || d.nom === 'comics-tracker'
        || d.hash.startsWith('ddl:')) {
        res.status(400).json({
          erreur: 'une release en lien direct ne s assemble pas : elle est deja un fichier',
        });
        return;
      }

      const svc = service(d.nom, d.cle);
      // Meme regle qu'ailleurs : assembler une archive n'est pas deposer.
      const id = (await svc.trouver(d.hash)) || (await svc.deposer(d.hash));
      if (!id) {
        res.status(502).json({
          erreur: `${d.nom} n a pas pu ouvrir cette release a partir de son empreinte seule. `
            + 'Utilisez « Deposer » si elle n est pas dans son cache.',
        });
        return;
      }
