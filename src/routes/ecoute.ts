...
async function ouvrirEcoute(
  ...
  // UNE RELEASE FOURTOUTICI EST UN FICHIER, PAS UNE LISTE DE PISTES. Toute cette surface
  // est batie sur une release qui en contient plusieurs — un identifiant de depot, un
  // numero de piste. Un fichier seul demanderait une autre mecanique ; la lui imposer
  // rendrait un « aucune piste » incomprehensible.
  // LibGen de meme : un fichier par md5, et aucun livre audio dans ses rubriques.
  if (d.nom === 'fourtoutici' || d.nom === 'libgen' || d.nom === 'comics-tracker') {
    throw new ErreurEcoute(400, 'cette source sert un fichier seul, pas une release a pistes');
  }
...
export function routesEcoute(db: Database): Router {
  ...
  r.get('/api/audio/:identite/piste/:n', exigerConnexion, async (req, res) => {
    ...
    try {
      ...
      if (estVerdict(d) || d.nom === 'telegram' || d.nom === 'fourtoutici' || d.nom === 'libgen'
        || d.nom === 'comics-tracker') {
        throw new ErreurEcoute(400, 'ecoute refusee');
      }
