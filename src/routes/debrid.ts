...
async function sonderVraiment(
  ...
  for (const [nom, cle] of configures) {
    ...
    const aEssayer = liensASonder(
      liens,
      (l) => !estRefuse(nom, l.url) && hoteExploitable(l.hebergeur, supportes[nom]),
    );
  ...
  // release.
  const quiARefuse = configures.length === 1
    ? configures[0]![0]
    : `vos ${configures.length} debrideurs`;
  return {
    format: 'inconnu',
    lisible: false,
    ...
    // ON DIT CE QU'ON A FAIT, ET CE QU'ON N'A PAS FAIT. Annoncer quatre echecs pour un
    // seul essai est une conclusion qu'on n'a pas gagnee, et elle ferme une release
    // peut-etre vivante.
    // ON NOMME CELUI QUI REFUSE, ET ON DIT CE QU ON A ESSAYE.
    //
    // « ne repondent plus chez leur hebergeur » accusait l'hebergeur d'un refus qui est
    // celui du DEBRIDEUR : verifie le 2026-09-02 sur un lien dailyuploads qui s'ouvre au
    // navigateur et qu'AllDebrid comme TorBox refusent. Le fichier est la ; ce sont les
    // debrideurs qui ne savent pas l'atteindre, et c'est une autre affaire — elle se
    // corrige en changeant de release, pas en attendant que l'hebergeur revienne.
    motif: essayes.size < prisEnCharge.length
      ? `${quiARefuse} n a pas su ouvrir ${essayes.size} lien(s) essaye(s) `
        + `(${prisEnCharge.length - essayes.size} autre(s) non testes : la sonde en essaie `
        + `${MAX_LIENS_SONDES} au plus)`
      : `${quiARefuse} n a su ouvrir aucun des ${prisEnCharge.length} lien(s) utilisables `
        + 'de cette release',
  };
}
