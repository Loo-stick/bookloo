...
async function sonderArchive(
  ...
  return { format: 'inconnu', lisible: false, motif: 'aucun hebergeur joignable pour sonder' };
}
...
export function routesDebrid(db: Database): Router {
  ...
  r.get('/api/release/:hash/fichiers', exigerConnexion, async (req, res) => {
  ...
  r.post('/api/release/:hash/fichiers-debrid', exigerConnexion, exigerCsrf, async (req, res) => {
    ...
    const id = await svc.deposer(hash, lienMemorise(req.session!.userId, hash));
    if (!id) {
  ...
  /** Depot assume : l'utilisateur a confirme, le telechargement peut etre differe. */
  r.post('/api/debrid/envoyer', exigerConnexion, exigerCsrf, async (req, res) => {
    const d = resoudreDemande(db, req);
    if (estErreur(d)) { res.status(d.code).json({ erreur: d.erreur }); return; }
    const { nom, hash, cle } = d;

    const torrentUrl = lienMemorise(req.session!.userId, hash);
    const id = await service(nom, cle).deposer(hash, torrentUrl);
    if (!id) {
      res.status(502).json({ accepte: false, differe: false, erreur: `${nom} a refuse le depot` });
      return;
    }
    tracer('Debrid', `depot accepte sur ${nom}`);
    res.json({ accepte: true, differe: true, id });
  });
  ...
  r.post('/api/debrid/liens', exigerConnexion, exigerCsrf, async (req, res) => {
    ...
    // Seed anti-H&R : si ce download vient de couter du ratio (source a passkey, pas
    // deja en cache), on pousse le torrent au Rustatio de l'utilisateur. Best-effort
    // ABSOLU : quoi qu'il arrive, le seed ne doit JAMAIS casser la reponse du download.
    ...
    res.json({ liens, demandes: voulus.length, resolus: liens.length, rustatio });
  });
  ...
  r.post('/api/debrid/zip', exigerConnexion, exigerCsrf, async (req, res) => {
    ...
    const id = await svc.deposer(hash, lienMemorise(req.session!.userId, hash));
    const url = id ? await svc.lienZip(id) : null;
