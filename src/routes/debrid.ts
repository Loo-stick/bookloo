...
import {
  liensASonder, verdictDesPremiersOctets, BUDGET_SONDAGE_MS, MAX_LIENS_SONDES,
} from './sondage';
...
import { ouvreurDdl, SITES_DDL } from '../sources/ddl/ouvreurs';
import { estIdentiteFourtoutici, idFourtoutici, urlFichierFourtoutici } from '../sources/ddl/fourtoutici';
import { estIdentiteLibgen, md5Libgen } from '../sources/ddl/libgen-liste';
import {
  PATIENCE_LIBGEN, prechargesLibgen, resolveurLibgen, resoudreFichierLibgen,
} from '../sources/ddl/libgen';
import { ouvrirLocal } from '../lecture/precharges';
import { estIdentiteCt, idCt } from '../sources/comics-tracker/recherche';
import { resolveurCt } from '../sources/comics-tracker/ouverture';
import { hotesSupportes } from '../debrid/hosts';
import {
  debloqueLesLiensDirects, declaresDabord, motifSansLienDirect, motifSondeEchouee,
} from '../debrid/tentable';
import { estRefuse, refuseDepuis } from '../debrid/refus-debrideur';
import { resoudreDdl } from '../debrid/ddl';
import { ouvrirDistant } from '../lecture/plages';
import { verdictRar } from '../lecture/rar';
import { estEpub, inspecterZip } from '../lecture/zip';
...
import { familleNumerote, identiteTome } from '../bibliotheque/identites';
import { numeroDeFichier } from '../sources/torrent/structure';
import { recupererTorrent } from '../debrid/recuperer-torrent';
...
export function apparierDebrid<T extends { nom: string }>(
  ...
  // LE CHEMIN COMPLET D'ABORD, le nom court seulement s'il est unique. Indexer sur le seul
  // nom de base faisait que « Tome1/x.cbz » masquait « Tome2/x.cbz » : on deposait alors le
  // mauvais tome, sans erreur ni signal. Signale par une revue de code le 2026-08-29.
  const out: T[] = [];
  for (const c of cheminsVoulus) {
    const f = choisirParChemin(cotesDebrid, c);
    if (f) out.push(f);
  }
  return out;
}
...
async function peutEtreSeeder(
  ...
  if (!source || !doitSeeder({ source, aussiSur, aConfigRustatio: !!cfg, torrentUrl, etatCache })) {
    ...
    const why = !source ? 'source-non-memorisee'
      ...
      // 'cache-inconnu' ne peut plus apparaitre ici : depuis le 2026-08-29 un etat inconnu
      // SEEDE au lieu d'etre ignore. Le motif est retire plutot que garde « au cas ou » —
      // un motif que rien ne produit ferait chercher une cause qui n'existe pas.
      : 'inconnu';
  ...
  // PAR LE MODULE GARDE. L'URL vient d'un tracker : sans verification, ce serveur
  // devient un relais vers son propre reseau, et les octets partent ensuite chez
  // Rustatio, ou ils deviennent lisibles.
  const rep = await recupererTorrent(torrentUrl, {
    timeoutMs: 15000, maxOctets: MAX_TORRENT_OCTETS,
  });
  if (!rep || rep.statut >= 400) {
    tracer('Rustatio', `seed echec: torrent-illisible (source=${source})`);
    return { statut: 'echec', raison: 'torrent-illisible' };
  }
  const buf = rep.octets;
...
export interface Archive {
  ...
  /**
   * La sonde a essaye TOUS les liens utilisables et aucun n'a repondu.
   *
   * DISTINCT DE `lisible: false`, qui dit seulement « la visionneuse ne sait pas
   * l'ouvrir » — un RAR ou un pack de tomes se telecharge tres bien. Ici c'est la release
   * qui est morte, et proposer de la telecharger promet ce qui vient d'echouer.
   *
   * Ce champ existe parce que la conclusion ne vivait que dans `motif`, une phrase que
   * seul un humain peut lire : l'interface affichait donc le motif ET le bouton.
   */
  aucunLienVivant?: boolean;
  motif?: string;
}
...
async function sonderArchive(
  ...
  // DEPASSER LE BUDGET N'EST PAS UNE ERREUR, C'EST UNE REPONSE. La sonde ne repond qu'a
  // une question — « la visionneuse saura-t-elle ouvrir ca ? » — et quand elle ne peut
  // pas trancher a temps, « telechargez pour voir » est un resultat honnete. Sans cette
  // borne, la requete depassait le delai du proxy et l'utilisateur recevait un 502 nu.
  return Promise.race([
    sonderVraiment(liens, supportes, cles),
    new Promise<Archive>((resoudre) => {
      setTimeout(() => resoudre({
        format: 'inconnu',
        lisible: false,
        motif: 'le sondage a pris trop de temps — telechargez la release pour la lire hors ligne',
      }), BUDGET_SONDAGE_MS).unref();
    }),
  ]);
}

async function sonderVraiment(
  liens: { hebergeur: string; url: string }[],
  supportes: Record<string, Set<string>>,
  cles: { ad?: string; tb?: string },
): Promise<Archive> {
  ...
  // UN LIEN DIRECT NE SE PROPOSE QU'A QUI SAIT L'OUVRIR ICI. TorBox etait compte ici des
  // qu'il declarait l'hebergeur — or `resoudreDdl` ne parle qu'a AllDebrid, et TorBox
  // rendait null d'office. D'ou « vos 2 debrideurs n a su ouvrir aucun des 1 lien(s) » sur
  // un lien vivant, signale a l'usage. Voir `debrid/tentable.ts`.
  const capables = configures.filter(([nom]) => debloqueLesLiensDirects(nom));
  if (capables.length === 0) {
    return {
      format: 'inconnu',
      lisible: false,
      motif: configures.map(([nom]) => motifSansLienDirect(nom)).join(' ; '),
    };
  }

  // TOUS LES LIENS SONT UTILISABLES PAR UN SERVICE CAPABLE. Sa liste d'hebergeurs ordonne
  // les essais, elle ne les interdit plus : mesure du 2026-09-11, AllDebrid ouvre
  // `turbobita.net`, qu'elle ne cite pas, et d'un hebergeur qu'elle declare desactive.
  const prisEnCharge = liens;
  if (prisEnCharge.length === 0) {
    return { format: 'inconnu', lisible: false, motif: 'aucun lien sur cette fiche' };
  }

  // COMBIEN ONT REELLEMENT ETE ESSAYES. Le message annoncait le nombre de liens
  // UTILISABLES — « les 4 lien(s) de cette release ne repondent plus » — alors que la
  // sonde en essaie deux au plus. Sur quatre liens du meme hebergeur, elle n'en tentait
  // qu'un : on declarait morts trois liens jamais appeles.
  const essayes = new Set<string>();

  for (const [nom, cle] of capables) {
    // ON N'ESSAIE PAS TOUT, ET PAS SANS FIN. Voir `routes/sondage.ts` : depuis que bookys
    // resout ses liens par flaresolverr, chaque essai coute treize secondes, et la boucle
    // complete depassait le delai du proxy — qui rendait alors un 502 NU, sans message.
    // Les hebergeurs que le service DECLARE d'abord : a deux essais au plus, commencer par
    // ceux qui ont le plus de chances ne coute rien et evite des refus previsibles.
    const aEssayer = liensASonder(
      declaresDabord(liens, supportes[nom] ?? new Set<string>()),
      (l) => !estRefuse(nom, l.url),
    );
    for (const l of aEssayer) {
      essayes.add(l.url);
      ...
      try {
        ...
        // Le verdict des premiers octets vit dans `sondage.ts`, module pur : celui qui
        // etait ecrit ici refusait les RAR, ce qui n'est plus vrai depuis que le lecteur
        // sait les ouvrir.
        const verdict = verdictDesPremiersOctets(debut);
        if (verdict) return verdict;
        const contenu = await inspecterZip(distant.lire, distant.taille);
        // Meme regle pour toutes les sources : un EPUB a sa liseuse, pas la visionneuse.
        if (estEpub(contenu.noms)) return { format: 'epub', lisible: true };
  ...
  // Des hotes pris en charge, mais aucun lien qu'un debrideur sache ouvrir. Ce n'est pas
  // la configuration qui est en cause — l'interface peut donc proposer de changer de
  // release.
  return {
    format: 'inconnu',
    lisible: false,
    // ON LE DIT EN CLAIR, ET PAS SEULEMENT DANS UNE PHRASE. L'interface affichait ce
    // motif ET un bouton « Telecharger » juste en dessous, qui promettait exactement ce
    // qui venait d'echouer — signale a l'usage. Une conclusion que seul un humain peut
    // lire n'est pas une conclusion utilisable.
    aucunLienVivant: true,
    // Ce qu'on a essaye, par qui, et accorde : voir `motifSondeEchouee`.
    motif: motifSondeEchouee(
      capables.map(([nom]) => nom), essayes.size, prisEnCharge.length, MAX_LIENS_SONDES,
    ),
  };
}
...
export function routesDebrid(db: Database): Router {
  ...
  r.post('/api/release/:hash/fichiers', exigerConnexion, exigerCsrf, async (req, res) => {
    ...
    if (estIdentiteDdl(brut)) {
      ...
      // LE RAYON DECIDE SI UN NUMERO EST UN TOME. Un periodique n'en a pas : voir
      // `identiteTome`. Absent, on numerote comme avant — se taire ne doit pas
      // reecrire une identite deja posee en base.
      const numerote = familleNumerote(req.query.famille);

      // UNE SOURCE QUI SERT SES FICHIERS N'A NI FICHE NI HEBERGEUR : l'identite EST le
      // fichier. Toute la suite cherche « le meme fichier a plusieurs endroits », ce qui
      // n'a pas de sens ici — et la table des ouvreurs rendait donc « source inconnue »
      // sur une release parfaitement lisible.
      if (estIdentiteFourtoutici(brut) || estIdentiteLibgen(brut) || estIdentiteCt(brut)) {
        const titreDirect = String(req.query.titre ?? '').trim() || brut;
        // LIBGEN SUIT CE CHEMIN, avec une adresse qui se RESOUT au lieu d'etre connue :
        // ads.php, puis get.php, puis le CDN. La sonde la resout donc une fois ; la lecture qui
        // suit reprend le lien garde, sans resolliciter LibGen. Voir `sources/ddl/libgen.ts`.
        const libgen = estIdentiteLibgen(brut);
        // COMICS TRACKER AUSSI, avec le compte du membre : son adresse Drive est signee pour
        // cette demande-la, et ne se devine pas.
        const ct = estIdentiteCt(brut);
        const resoudre = ct
          ? resolveurCt(
            req.session!.userId,
            clesEnClair(db, req.session!.userId, req.session!.dek),
            idCt(brut),
          )
          : libgen
            ? resolveurLibgen(md5Libgen(brut))
            : async () => urlFichierFourtoutici(idFourtoutici(brut));
        let url: string | undefined = libgen || ct ? undefined : urlFichierFourtoutici(idFourtoutici(brut));
        let archive: Archive = { format: 'inconnu', lisible: false };
        let taille: number | undefined;
        try {
          // UN FICHIER LIBGEN DEJA PRECHARGE SE SONDE SUR LE DISQUE : rouvrir depuis la
          // bibliotheque n'a pas a attendre le CDN pour lire huit octets qu'on possede.
          const local = libgen ? prechargesLibgen().cheminPret(md5Libgen(brut)) : null;
          const distant = local ? await ouvrirLocal(local) : await ouvrirDistant({
            resoudre: async () => {
              const u = await resoudre();
              if (u) url = u;
              return u;
            },
            // Le CDN de LibGen est lent mais vivant : voir `PATIENCE_LIBGEN`.
            patience: libgen ? PATIENCE_LIBGEN : undefined,
          });
          taille = distant.taille;
          const debut = (await distant.lire(0, 7)).toString('latin1');
          const verdict = verdictDesPremiersOctets(debut);
          // UN RAR SE JUGE PLUS FINEMENT ICI, en deux ou trois lectures de plus : compresse,
          // il ne se lit pas en ligne, et « Lire » echouait neuf secondes apres le clic.
          if (verdict?.format === 'rar') archive = await verdictRar(distant.lire, distant.taille);
          else if (verdict) archive = verdict;
          else {
            const contenu = await inspecterZip(distant.lire, distant.taille);
            // UN EPUB EST UN ZIP : sans ce test, un roman partait dans la visionneuse, qui
            // y trouvait la couverture et une illustration et annoncait « 2 planches ».
            if (estEpub(contenu.noms)) archive = { format: 'epub', lisible: true };
            else if (contenu.images.length > 0) {
              archive = { format: 'zip', lisible: true, nbPlanches: contenu.images.length };
            } else {
              archive = { format: 'zip', lisible: false, motif: 'archive sans planche' };
            }
          }
        } catch (e) {
          archive = { format: 'inconnu', lisible: false, motif: (e as Error).message };
          // LIBGEN N'A QU'UN LIEN : s'il ne s'ouvre pas, proposer « Telecharger » promettrait
          // ce qui vient d'echouer. L'interface montre alors la raison a la place des boutons.
          if (libgen || ct) archive.aucunLienVivant = true;
        }
        res.json({
          type: 'hebergeurs',
          archive,
          hebergeurs: [{
            index: 0,
            hebergeur: ct ? 'comics-tracker' : libgen ? 'libgen' : 'fourtoutici',
            taille,
            nomFichier: titreDirect,
            tomeId: identiteTome(
              { tome: numeroDeFichier(titreDirect), chemin: titreDirect },
              numerote,
            ),
            refusePar: [],
            url,
            // AUCUN DEBRIDEUR N'EST NECESSAIRE, et le dire vaut mieux que pretendre que
            // les deux le prennent en charge : l'interface saurait quoi afficher, mais
            // pour une raison fausse.
            sansDebrideur: true,
            supporte: { alldebrid: true, torbox: true },
          }],
        });
        return;
      }

      // LA TABLE EST COMMUNE AVEC `routes/lecture.ts`. Elle vivait ici en dur — un
      // `!== 'wawacity'` — et une release bookys parfaitement trouvee refusait donc de
      // s'ouvrir. Le message nomme desormais ce qu'il connait, pour que l'ecart se voie.
      const ouvreur = ouvreurDdl(decoupe.site);
      if (!ouvreur) {
        res.status(400).json({
          erreur: `source « ${decoupe.site} » inconnue (connues : ${SITES_DDL.join(', ')})`,
        });
        return;
      }
      // Le titre que le client affiche, pour nommer un fichier que l'hebergeur ne nomme
      // pas. Vide, on retombe sur l'identite : jamais sur une chaine vide, qui ferait
      // une identite `f:` sans nom.
      const titreRelease = String(req.query.titre ?? '').trim() || brut;
      const liens = await ouvreur(decoupe.id);
      ...
      const cles = clesEnClair(db, req.session!.userId, req.session!.dek);
      // La liste de TorBox n'est plus demandee ici : il ne debloque pas de lien direct
      // dans cette application, et sa liste ne changerait rien. Celle d'AllDebrid ORDONNE
      // les essais de la sonde. Voir `debrid/tentable.ts`.
      ...
      res.json({
        ...
        hebergeurs: liens.map((l, index) => ({
          ...
          // L'IDENTITE DU TOME VIENT DU SERVEUR, comme pour une release de debrideur.
          //
          // Le client la refabriquait en `f:<nom>` — juste sous le commentaire qui
          // l'interdit : « `tomeId` vient du SERVEUR, le client ne recalcule pas cette
          // identite, sous peine de la voir diverger en silence ». Elle a diverge :
          // « Naruto (Édition Hokage) - Tome 01 (2022) » etait lu sous
          // `f:Naruto (Édition Hokage)…` au lieu de `t1`, et la bibliotheque ne savait
          // donc pas que le tome 1 avait ete lu — « il n'a pas memorise le tome 1, je
          // suis oblige de le chercher a nouveau ».
          //
          // Le titre de la release sert de repli quand l'hebergeur ne nomme pas son
          // fichier : c'est le meme nom que le client affichait deja.
          tomeId: identiteTome(
            {
              tome: numeroDeFichier(l.nomFichier || titreRelease),
              chemin: l.nomFichier || titreRelease,
            },
            numerote,
          ),
          // LE REFUS EST NOMME PAR SON AUTEUR. « lien mort » etait le mot d'AllDebrid
          // repris a notre compte : il dit qu'AllDebrid n'a pas su ouvrir ce lien, pas
          // que le fichier a disparu. Un lien parfaitement vivant restait affiche mort.
          refusePar: (['alldebrid', 'torbox'] as const).filter((n) => estRefuse(n, l.url)),
          refuseDepuis: (['alldebrid', 'torbox'] as const)
            .map((n) => refuseDepuis(n, l.url))
            .find(Boolean)?.toISOString()
            .slice(0, 10),
          // L'ADRESSE PUBLIQUE DE L'HEBERGEUR, ET ELLE SEULE.
          //
          // La regle « le lien protege ne part pas vers le navigateur » vise le lien
          // DEBRIDE, qui porte le compte de l'utilisateur — celui-la reste designe par son
          // index, quelques lignes plus bas. Ici c'est l'adresse que la fiche affiche deja
          // a tout le monde : aucun secret, et c'est la seule sortie quand les debrideurs
          // refusent tous. Un fichier qui s'ouvre au navigateur ne doit pas laisser
          // l'ecran sans aucun geste a proposer.
          //
          // Elle n'est envoyee que pour une release en LIEN DIRECT : cette reponse-la
          // n'existe pas pour un torrent, ou l'adresse porterait la passkey du tracker.
          url: /^https?:\/\//i.test(l.url) ? l.url : undefined,
          // Le lien protege NE PART PAS vers le navigateur : il est designe par son index.
          // CE QUE CHAQUE SERVICE PEUT ESSAYER, et pas ce que sa liste declare : celle
          // d'AllDebrid est incomplete, et TorBox n'a pas ce chemin ici. Le verdict reel
          // vient de l'essai, et un refus reste memorise par lien (`refusePar`).
          supporte: {
            alldebrid: Boolean(cles.ad) && debloqueLesLiensDirects('alldebrid'),
            torbox: Boolean(cles.tb) && debloqueLesLiensDirects('torbox'),
          },
          // ET POURQUOI, quand ce n'est pas le cas : « non pris en charge par torbox »
          // aurait ete faux — TorBox prend cet hebergeur, c'est l'application qui ne sait
          // pas encore le lui demander.
          pourquoi: {
            alldebrid: !cles.ad ? 'aucune cle alldebrid enregistree' : motifSansLienDirect('alldebrid'),
            torbox: !cles.tb ? 'aucune cle torbox enregistree' : motifSansLienDirect('torbox'),
          },
        })),
      });
      return;
    ...
    // Meme garde : l'URL du .torrent vient du tracker, pas de nous.
    const reponse = await recupererTorrent(url, { timeoutMs: 20000, maxOctets: MAX_TORRENT_OCTETS });
    if (!reponse || reponse.statut >= 400) {
      res.status(502).json({ erreur: 'le tracker n a pas rendu le .torrent' });
      return;
    }
    const fichiers = fichiersDuTorrent(reponse.octets);
    if (!fichiers) {
  ...
  r.post('/api/debrid/lien-direct', exigerConnexion, exigerCsrf, async (req, res) => {
    ...
    // LES SOURCES QUI SERVENT LEURS PROPRES FICHIERS PASSENT AVANT LA CLE. Elles n'ont besoin
    // d'aucun debrideur, et le garde-fou ci-dessous repondait pourtant « aucune cle alldebrid
    // enregistree » sur un fichier parfaitement telechargeable — un refus pour une raison fausse.
    //
    // SANS DEBRIDEUR, l'adresse est deja la bonne : rien a deverrouiller, rien a essayer.
    if (estIdentiteFourtoutici(identite)) {
      res.json({
        url: urlFichierFourtoutici(idFourtoutici(identite)),
        hebergeur: 'fourtoutici',
        repli: false,
        essayes: [],
      });
      return;
    ...
    // LIBGEN DE MEME, une fois son adresse resolue. Un echec se DIT : la base de LibGen refuse
    // des connexions par moments, et le geste utile est de reessayer, pas de changer de source.
    if (estIdentiteLibgen(identite)) {
      try {
        const url = await resoudreFichierLibgen(md5Libgen(identite));
        if (!url) { res.status(400).json({ erreur: 'identite invalide' }); return; }
        res.json({ url, hebergeur: 'libgen', repli: false, essayes: [] });
      } catch (e) {
        // La raison vient du resolveur : CDN en panne, ou base de LibGen muette.
        res.status(502).json({ erreur: (e as Error).message });
      }
      return;
    }

    // COMICS TRACKER : son adresse se demande avec le compte du membre, et Google Drive la
    // signe pour cette demande. Un echec se DIT — compte, edition, ou quota Drive appellent
    // trois gestes differents.
    if (estIdentiteCt(identite)) {
      try {
        const clesCt = clesEnClair(db, req.session!.userId, req.session!.dek);
        const url = await resolveurCt(req.session!.userId, clesCt, idCt(identite))();
        res.json({ url, hebergeur: 'comics-tracker', repli: false, essayes: [] });
      } catch (e) {
        res.status(502).json({ erreur: (e as Error).message });
      }
      return;
    ...
    // CE CHEMIN IGNORAIT LE SITE DE L'IDENTITE et ouvrait toujours une fiche Wawacity.
    // Pour « ddl:bookys:334943 » il serait alle chercher la fiche 334943 DE WAWACITY —
    // une release sans aucun rapport, servie sans que rien ne le signale. Un mauvais
    // contenu rendu en silence est pire qu'un refus.
    const ouvreur = ouvreurDdl(decoupe.site);
    if (!ouvreur) {
      res.status(400).json({
        erreur: `source « ${decoupe.site} » inconnue (connues : ${SITES_DDL.join(', ')})`,
    ...
    // Seul un service qui sait debloquer un lien direct ICI peut servir : voir
    // `debrid/tentable.ts`. Le dire vaut mieux que le laisser echouer apres coup.
    const sansChemin = motifSansLienDirect(nom);
    if (sansChemin) { res.status(400).json({ erreur: sansChemin }); return; }

    const supportes = await hotesSupportes(nom, cle);
    // On commence par celui demande, puis les autres — ceux que le service DECLARE d'abord.
    // Sa liste ordonne, elle n'interdit plus : elle est incomplete, et sa reponse fait foi.
    const indexes = liens.map((l, i) => ({ hebergeur: l.hebergeur, i }));
    const ordre = [
      ...indexes.filter((x) => x.i === depuis),
      ...declaresDabord(indexes.filter((x) => x.i !== depuis), supportes),
    ].map((x) => x.i);
    const essayes: string[] = [];

    for (const i of ordre) {
      const l = liens[i];
      if (estRefuse(nom, l.url)) continue;
    ...
    res.status(502).json({
      // Aucun essai : tous les liens avaient deja ete refuses par ce service.
      erreur: essayes.length === 0
        ? `${nom} a deja refuse tous les liens de cette fiche`
        : `${nom} n a su ouvrir aucun lien (${essayes.join(', ')})`,
      essayes,
    });
  });
