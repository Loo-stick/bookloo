...
export function routesEmpreinte(): Router {
  ...
  r.post('/api/empreinte', exigerConnexion, exigerCsrf, limiteurRecherche, async (req, res) => {
    ...
    try {
      // Meme garde que les depots chez les debrideurs : l'URL vient du tracker.
      const rep = await recupererTorrent(url, { timeoutMs: 15000, maxOctets: MAX_OCTETS });
      if (!rep || rep.statut !== 200) {
        // 429 : le tracker limite le debit. Le DIRE, plutot que « empreinte
        // indisponible » — le geste a refaire n'est pas le meme.
        const message = rep?.statut === 429
      ...
      const hash = infoHashDepuisTorrent(rep.octets);
